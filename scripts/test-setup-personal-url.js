...
function fixture(respond = () => ({ state: 'off' })) {
  ...
  return { api, calls, copied, storage, node: id => node('#' + id),
...
test('URL validation and preview precede owner confirmation; only managed enrollment is requested', async () => {
  ...
  assert.equal(f.node('url-confirm-wrap').hidden, false); assert.equal(f.api.settled(), false);
...
test('unsafe returned URLs are never rendered as navigation targets', async () => {
  ...
  assert.equal(f.node('url-confirm-wrap').hidden, true); assert.equal(f.node('url-saved').hidden, true);
});
