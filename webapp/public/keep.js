(() => {
  ...
  $('send').onclick = async () => {
    $('send').disabled = true;
    try { const r = await fetch('/api/phone/send', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }); const d = await r.json(); if (!r.ok) throw new Error(d.error); message('The link is queued for your connected chat.'); }
