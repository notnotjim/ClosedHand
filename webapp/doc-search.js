// webapp/doc-search.js (vendored from lib/services/doc-search.js) — hybrid retrieval over the File Search index
...
function docSearch({ supabase, embed, rerank }) {
   ...
   */
  async function searchDocuments(userId, query, { candidatesOnly = false } = {}) {
    ...
    if (docIds.length > 0) {
      const { data: docs, error } = await supabase.from("rag_documents")
        .select("id, name, origin, file_path, updated_at")
        .eq("user_id", userId).eq("status", "ready").in("id", docIds);
      if (error) return { error: error.message };
      for (const d of docs || []) docMap[d.id] = d;
    }

    let results = merged.filter(({ row }) => docMap[row.document_id]).map(({ row: m, arms }) => {
    ...
    // Passive recall ranks file passages together with mail and other sources.
    // Keep candidate collection shared without paying for two reranking calls.
    if (candidatesOnly) return { results: results.slice(0, 30) };
