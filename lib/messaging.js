...
const SLACK_BOT_TOKEN = () => process.env.SLACK_BOT_TOKEN || require("./config").getConfCached("SLACK_BOT_TOKEN");
...
function sendWhatsAppMessage(to, text) {
  ...
  text = require("./chat-format").formatWhatsApp(text);
...
async function sendWhatsAppDocument(chatId, buffer, filename, mimeType, inlineImage = false) {
  if (!WHATSAPP_ACCESS_TOKEN || !WHATSAPP_PHONE_NUMBER_ID) {
  ...
  // Only generated previews opt in. Original files retain document delivery.
  ...
  const postData = JSON.stringify({
    ...
    to: chatId,
    ...(preview
      ? { type: "image", image: { id: mediaResult.id, caption: filename } }
      : { type: "document", document: { id: mediaResult.id, filename } }),
  });
...
async function sendDocument(chatId, buffer, filename, mimeType, origin) {
  ...
  switch (ctx.activePlatform) {
    case "email":
      return require("./assistant-email").send(chatId, "", { filename, contentType: mimeType, content: buffer.toString("base64") });
    case "web":
...
function sendTyping(chatId) {
  if (ctx.activePlatform === "email") return Promise.resolve();
...
function sendText(chatId, text) {
  if (ctx.activePlatform === "email") return require("./assistant-email").send(chatId, text);
...
async function sendDocumentToPlatform(platform, chatId, buffer, filename, mimeType) {
  if (platform === "email") return require("./assistant-email").send(chatId, "", { filename, contentType: mimeType, content: buffer.toString("base64") });
...
async function sendToPlatform(platform, chatId, message) {
  if (platform === "email") return require("./assistant-email").send(chatId, message);
  if (platform === "dashboard") return null; // Dashboard polls for results
  switch (platform) {
    case "telegram": {
...
async function sendCanvasToChat(platform, chatId, canvasUrl, filename, buffer, mimeType) {
  ...
  switch (platform) {
    case "whatsapp":
      if (buffer) await sendWhatsAppDocument(chatId, buffer, filename, mimeType, true);
      return;
    case "whatsapp_linked": {
      const { sendLinkedDocument } = require("./platforms/whatsapp-linked");
      if (buffer) await sendLinkedDocument(chatId, buffer, filename, mimeType, true);
      return;
    }
    case "telegram": {
    ...
    case "line": {
      ...
      const liffId = process.env.LINE_LIFF_ID || require("./config").getConfCached("LINE_LIFF_ID");
      const lineToken = process.env.LINE_CHANNEL_ACCESS_TOKEN || require("./config").getConfCached("LINE_CHANNEL_ACCESS_TOKEN");
      if (liffId && lineToken) {
