...
async function finishReply(db, row, deps) {
  ...
  try {
    ...
    // Quick structured work: thinking off where the provider allows it, or the
    // reasoning eats the cap and the parked answer is dropped as "superseded".
    const response = await withTaskRun({ userId: info.userId, taskId: info.requestId, kind: "followup" }, () => modelCall(current.client, {
      model: current.model, max_tokens: 1000, effort: "fast",
    ...
    if (reply?.send !== true || typeof reply.answer !== "string" || !reply.answer.trim()) {
      console.log(`[deferred-reply] not sending ${info.requestId}: ${reply ? "the model chose not to (" + String(reply.reason || reply.why || "no reason given").slice(0, 160) + ")" : "no parseable verdict"}`);
