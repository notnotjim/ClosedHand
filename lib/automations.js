...
RULES:
- Full access to connected services, with one exception: an automation never pays for anything. If something needs paying, say exactly what and how much in the report and the user will do it in chat.
      ...
      const result = await acquireUserMutex(userId, async () => {
        ...
        try {
          if (isInternalTool(block.name)) {
            ...
            if (spendGuard.spendIntent(block.name, input)) {
              await spendGuard.ledgerInsert({ user_id: userId, status: "refused", source: "automation", tool: block.name, note: "automations never pay" });
              toolResult = { error: "Not done. An automation never pays for anything. Put what needs paying, and how much, in the report; the user will do it in chat." };
            } else {
