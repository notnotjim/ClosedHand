...
const { describeAgentProgress, formatElapsed } = (() => {
  ...
  const TIPS = [
    ...
    "You can review and delete what ClosedHand remembers in Context Brain on your dashboard.",
    ...
    "Connect more services on your dashboard to give ClosedHand new capabilities.",
    "You can set reminders, track flights, or manage your calendar, just by asking.",
    "Try asking ClosedHand to draft emails, manage your Shopify store, or research a topic.",
    "ClosedHand's memory is transparent. Check the Context Brain tab on your dashboard to see exactly what it knows.",
    ...
    "Everything ClosedHand remembers is deletable from your dashboard. Nothing is permanent unless you want it to be.",
...
3. VERIFY: Before finishing, check your work.
   ...
   - Correct a fixable issue within the task budget. If it remains unresolved, say so rather than repeatedly rewriting or claiming success.
4. DELIVER the answer or update directly, usually in a few sentences or a short list. A scheduled task describes WHEN to work, not a requirement to produce a report. Summarise sources accurately and retain important numbers, sources and caveats. Quote exact wording when requested or when it matters. Create a full report or separate file only when requested or explicitly needed by the task. Do not add executive summaries or recommendations to ordinary updates.
...
- Keep your final response focused on results, not process or tool names. Surface failures and needed user input honestly.
- Batch related sandbox_browse actions, observing the page before choosing dependent actions, to avoid repeated browser connections.
...
async function _executePreparedLoop(runId, config, userId, platform, chatId, model, inputContext, successCriteria, resumeRow) {
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    const response = await require("./agent-context").createAgentResponse(getUserLLMClient(userId, userStore).client, {
      model: fullModel, effort: TIER_MAP[model] || "default", max_tokens: 4096, system: systemPrompt, messages, tools: toolScope.definitions(messages),
    }, provider);
    await require("./task-lease").renew(require("./db").supabase, "automation_runs", runId);
...
No explanation, just JSON.`,
  ...
  try {
    ...
    return { error: "Failed to parse description", raw: responseText(resp) };
  }
}
