...
class PgQueryBuilder {
  ...
  // PostgREST projections can name the returned field: mfrom:data->>from.
  // The alias is an identifier, separate from the column/JSON expression.
  _selectRef(entry, hasEmbed = false) {
    const alias = String(entry).match(/^([A-Za-z_][A-Za-z0-9_]*):([^:].*)$/);
    if (!alias) return this._colRef(entry, hasEmbed);
    return this._colRef(alias[2], hasEmbed) + " AS " + quoteIdent(alias[1]);
  }
    ...
    if (this._countHead) {
      ...
      for (const e of entries) {
        ...
        if (m && embedMap[m[1]]) {
          ...
          cols.push(this._selectRef(e, hasEmbed));
        }
      }
      selectSql = cols.join(", ");
    }
