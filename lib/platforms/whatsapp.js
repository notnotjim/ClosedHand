          ...
          try {
            ...
            if (isImage) {
              ...
              try {
                ...
                // Buffer this image, wait for more
                if (!_waImageBuffer[waId]) {
                  _waImageBuffer[waId] = { images: [], timer: null, msgIds: [], userId, caption: null };
                }
                _waImageBuffer[waId].images.push({ buffer, mimeType: mimeType || msg.image.mime_type });
                _waImageBuffer[waId].msgIds.push(waMessageId);
                ...
                _waImageBuffer[waId].timer = setTimeout(() => {
                  ...
                  // Take a fresh turn on the queue, which is what gives this work
                  // a live context bubble of its own. The bubble this timer was
                  // scheduled inside belongs to the webhook that queued the last
                  // image, and that handler's finally has already run
                  // cleanupUserContext() on it. AsyncLocalStorage hands the timer
                  // the same object, so by the time it fires the platform, user,
                  // chat and store are all nulled: everything downstream then read
                  // a null platform and defaulted it to "web", and an agent spawned
                  // from an image turn reported to a web session that does not
                  // exist instead of back to WhatsApp.
                  queueUserMessage(batch.userId, async () => {
                    ...
                    try {
                      ...
                      if (batch.images.length === 1) {
                        ...
                        // Multi-image: pass as multiImage array
                        fileData = {
                          isMultiImage: true,
                          ...
                          ext: "jpg", isImage: true, fileName: `images_${batch.images.length}.jpg`,
                        };
                      }
