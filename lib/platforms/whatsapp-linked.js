...
const { archiveThread, switchThread, getThreadList, clearAllThreads, formatRelativeTime, getConversation } = require("../conversation");
...
// Messages already answered, by id, kept with the connection so a reconnect
// or restart cannot answer one twice. A message sent while the machine was
// asleep arrives on reconnect as a replay, not a live notification; it used to
// be filed for recall and never answered.
const _handled = new Set();
const HANDLED_KEEP = 300;
const TURN_MAX_AGE_MS = 12 * 3600 * 1000;
async function rememberHandled(id) {
  _handled.add(id);
  const list = Array.from(_handled).slice(-HANDLED_KEEP);
  _handled.clear(); for (const x of list) _handled.add(x);
  await updateConnMeta({ handled: list });
}
// What was sent, by id. A phone that could not read a message asks the sender
// for it again; without this the request cannot be answered and the phone
// shows "Waiting for this message" for good.
let _outbox = new Map();

// The signal library prints whole session records, private keys included,
// through console.info when it rotates a session. Those two lines are the
// only thing it says that way; they must not land in the log.
const _info = console.info;
console.info = function (first, ...rest) {
  if (typeof first === "string" && /^(Closing session|Removing old closed session)/.test(first)) return;
  return _info.call(console, first, ...rest);
};
let _historyIngested = 0;
let _imageBatch = null;    // { images: [{buffer, mimeType}], keys: [], caption, timer }
let _lastRealMsgAt = 0;
let _welcoming = false;
...
// WhatsApp wraps some messages (disappearing, view once, a document with a
// caption, an edit) in an outer message; the content is one level down.
function unwrap(m) {
  let x = m || {};
  for (let i = 0; i < 4; i++) {
...
// The kinds of message a person sends. Anything else in the self-chat is
// WhatsApp's own housekeeping (key shares, sync notices, receipts) and is
// not something to answer, let alone with a capability list.
const USER_CONTENT = ["conversation", "extendedTextMessage", "imageMessage", "audioMessage", "documentMessage",
  "videoMessage", "stickerMessage", "contactMessage", "contactsArrayMessage", "locationMessage", "liveLocationMessage", "pollCreationMessage"];

// A forwarded message carries WhatsApp's own flag on the inner content. It
// matters more here than anywhere: in the self-chat everything is "from
// me", so a forward from someone else reads as the user's own words, and a
// line like "let me know what you think tomorrow" inside it was once taken
// as an instruction to ClosedHand. The marker below says what it is.
...
function isForwarded(msg) {
  ...
  for (const k of Object.keys(m || {})) {
    const ci = m[k] && typeof m[k] === "object" ? m[k].contextInfo : null;
    if (ci && (ci.isForwarded === true || Number(ci.forwardingScore) > 0)) return true;
  }
  return false;
}
function withForwardTag(text, msg) {
  if (!isForwarded(msg)) return text;
...
function extractQuoted(msg) {
  const ci = unwrap(msg.message)?.extendedTextMessage?.contextInfo;
...
function inSelfChatTurn(fn, options = {}) {
...
async function openConversationIfNew() {
  if (_welcoming) return;
  _welcoming = true;
  try {
    await inSelfChatTurn(async (userId) => {
      const { data: row, error } = await supabase.from("connections").select("metadata")
        .eq("user_id", userId).eq("service", "whatsapp_linked").single();
      if (error) throw new Error(error.message);
      if (!row?.metadata?.linked || row.metadata.welcome?.sentAt) return;
      let welcome = row.metadata.welcome;
      if (!welcome) {
        welcome = { id: require("crypto").randomBytes(16).toString("hex").toUpperCase(),
          text: require("../setup-welcome").buildWelcome(ctx.store.facts) };
        const phoneUrl = await require("../config").getConf("PHONE_PERMANENT_URL");
        const phoneEnabled = await require("../config").getConf("PHONE_ACCESS");
        if (phoneUrl && String(phoneEnabled) === "1" && await require("../config").getConf("PHONE_ACCESS_MODE") === "managed") welcome.text += `\n\nSave your personal dashboard to your phone’s home screen here: ${phoneUrl}/keep`;
        const { error: saveError } = await supabase.from("connections")
          .update({ metadata: { ...row.metadata, welcome } })
          .eq("user_id", userId).eq("service", "whatsapp_linked");
        if (saveError) throw new Error(saveError.message);
      }
      // A stable WhatsApp message id makes retries refer to the same send.
      await sendLinkedMessage(_selfJid, welcome.text, welcome.id);
      const { error: sentError } = await supabase.from("connections")
        .update({ metadata: { ...row.metadata, welcome: { ...welcome, sentAt: new Date().toISOString() } } })
        .eq("user_id", userId).eq("service", "whatsapp_linked");
      if (sentError) throw new Error(sentError.message);
      getConversation(userId).push({ role: "assistant", content: welcome.text });
      saveStore();
      console.log("[wa-linked] setup welcome sent");
    }, { housekeeping: true });
  } catch (e) {
    console.warn("[wa-linked] Welcome could not be sent; will retry:", e.message);
  } finally { _welcoming = false; }
}

// One short message pinned at the top of the chat: the dashboard link and
// the shortcuts. Sent once per link, again only if its words change, and
// pinned for the longest WhatsApp allows, then pinned again when that lapses.
function shortcutsText(url) {
  return ["I’m pinning this so your dashboard and shortcuts are easy to find.", "", url ? `Your dashboard: ${url}` : "Your dashboard opens on the computer running ClosedHand.", "",
    "/new  start a fresh conversation", "/threads  go back to an earlier one", "/bug  report a problem"].join("\n");
}
async function pinInSelfChat(messageId, pin) {
  await _sock.sendMessage(_selfJid, { pin: { remoteJid: _selfJid, fromMe: true, id: messageId }, type: pin ? 1 : 2, time: 30 * 86400 });
}
async function pinShortcuts() {
  await inSelfChatTurn(async (userId) => {
    const { data: row } = await supabase.from("connections").select("metadata")
      .eq("user_id", userId).eq("service", "whatsapp_linked").single();
    if (!row?.metadata?.linked) return;
    const base = await require("../config").dashboardBase();
    const prior = row.metadata.shortcuts;
    // No address right now is a moment's failure, not a change of address:
    // a message already pinned stays, and a first message waits for it when
    // phone access is on. Sending the linkless version here pinned a message
    // with no link at all, then a second one minutes later.
    if (!base) {
      if (prior?.messageId) return;
      if (String(await require("../config").getConf("PHONE_ACCESS")) === "1") return;
    }
    const text = shortcutsText(base ? base + "/dashboard" : null);
    // Merged at write time, so a handled-id write landing meanwhile is kept.
    const save = (shortcuts) => updateConnMeta({ shortcuts });
    if (prior?.messageId && prior.text === text) {
      if (Date.now() - new Date(prior.pinnedAt || 0).getTime() < 29 * 86400000) return;
      await pinInSelfChat(prior.messageId, true);
      await save({ ...prior, pinnedAt: new Date().toISOString() });
      return;
    }
    const messageId = await sendLinkedMessage(_selfJid, text);
    if (!messageId) return;
    if (prior?.messageId) { try { await pinInSelfChat(prior.messageId, false); } catch (_) {} }
    await pinInSelfChat(messageId, true);
    await save({ messageId, text, sentAt: new Date().toISOString(), pinnedAt: new Date().toISOString() });
    console.log("[wa-linked] shortcuts pinned");
  }, { housekeeping: true });
}
  ...
  await inSelfChatTurn(async (userId) => {
    ...
    if (media === "image") {
      ...
      try {
        ...
        bufferImage(buffer, m.mimetype || "image/jpeg", withForwardTag(m.caption || "", msg) || null, key);
      } catch (e) {
        react(key, "");
    ...
    if (media === "document") {
      ...
      try {
        ...
        if (fileData.isImage) {
          bufferImage(buffer, fileData.mediaType, withForwardTag(m.caption || "", msg) || null, key);
          return;
        }
        await presence("composing");
        const response = await queuedAsk(userId, withForwardTag(m.caption || "", msg) || null, fileData, _selfJid);
        await presence("paused");
        react(key, "✅");
        if (response) await sayInSelfChat(response);
      } catch (e) {
    ...
    const engineText = withForwardTag(quoted
...
async function startSocket(ownerUserId) {
  ...
  try {
    require("../baileys-pairing-fix").applyPairingFix();
    ...
    // WhatsApp refuses to pair a web client that announces a version it has
    // retired, and the phone reports that as a connection problem. Ask for
    // the current version each time the socket starts; fall back to the
    // library's default if the lookup fails.
    let version;
    ...
    const sock = makeWASocket({
      ...
      getMessage: async (key) => _outbox.get(key?.id),
    });
    _sock = sock;
    ...
    sock.ev.on("connection.update", async (u) => {
      ...
      if (u.connection === "open") {
        ...
        try {
          const { data: row } = await supabase.from("connections").select("metadata").eq("user_id", _ownerUserId).eq("service", "whatsapp_linked").single();
          for (const id of row?.metadata?.handled || []) _handled.add(id);
        } catch (_) { /* an empty set only risks one repeated answer */ }
        ...
        openConversationIfNew().then(() => pinShortcuts()).catch((e) => console.warn("[wa-linked] shortcuts:", e.message));
      }
    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
      for (let msg of messages || []) {
        try {
          ...
          // A live message, or a recent one replayed on reconnect (sent while
          // the machine slept), is a turn once; older replays are history.
          const sentAt = Number(msg.messageTimestamp || 0) * 1000;
          if (type !== "notify" && (!sentAt || Date.now() - sentAt > TURN_MAX_AGE_MS)) continue;
          if (_handled.has(msg.key.id)) continue;

          if (!text && !media) {
            if (!USER_CONTENT.some((k) => m[k])) continue; // housekeeping, not a message
          ...
          await rememberHandled(msg.key.id);
          console.log(`[wa-linked] self-chat ${media || "message"}${type !== "notify" ? " (received while away)" : ""}: "${(text || "").substring(0, 60)}"`);
...
// In a chat with yourself, both sides are you. Every message ClosedHand
// sends opens with the fist and its name, so its bubbles read as its own.
let _botName = null;
let _botNameAt = 0;
async function botName() {
  if (_botName && Date.now() - _botNameAt < 5000) return _botName;
  try {
    const { data } = await supabase.from("profiles").select("settings").eq("id", _ownerUserId).single();
    _botName = data?.settings?.bot_name || "ClosedHand";
  } catch (_) {
    _botName = _botName || "ClosedHand";
  }
  _botNameAt = Date.now();
  return _botName;
}
...
function remember(out) {
  if (out?.key?.id) {
    ...
    if (out.message) {
      _outbox.set(out.key.id, out.message);
      if (_outbox.size > 300) _outbox = new Map(Array.from(_outbox.entries()).slice(100));
    }
  }
  return out?.key?.id || null;
}

// Close the connection without unlinking, so the keys on disk are complete
// before the process ends. Logging out is only for the person turning it off.
async function closeForShutdown() {
  const s = _sock;
  if (!s) return;
  _sock = null;
...
async function sendLinkedMessage(chatId, message, messageId) {
  if (!_sock) throw new Error("WhatsApp (linked) is not connected");
  const jid = _gateSelfChat(chatId);
  const text = stamp(await botName(), require("../chat-format").formatWhatsApp(message));
...
async function sendLinkedDocument(chatId, buffer, filename, mimeType, inlineImage = false) {
  if (!_sock) throw new Error("WhatsApp (linked) is not connected");
  const jid = _gateSelfChat(chatId);
  const preview = inlineImage && /^(image\/png|image\/jpeg)$/.test(mimeType)
    && buffer.length <= 5 * 1024 * 1024;
  return remember(await _sock.sendMessage(jid, preview
    ? { image: buffer, caption: filename, mimetype: mimeType }
    : { document: buffer, fileName: filename, mimetype: mimeType || "application/octet-stream" }));
}
  ...
  const ensure = async () => {
    try {
      const { data, error } = await supabase.from("connections")
        .select("user_id, config, metadata").eq("service", "whatsapp_linked").limit(1);
      // An unavailable database says nothing about whether the user disabled WhatsApp.
      // Logging out here revokes the linked session and forces a fresh phone pairing.
      if (error || !Array.isArray(data)) {
        console.warn("[wa-linked] Could not check connection settings; keeping current session");
        return;
      }
      const row = data?.[0];
      if (row && row.config?.enabled && !_sock && !_starting) {
        ...
        await stopSocket();
      } else if (row?.config?.enabled && row.metadata?.linked && _sock && !row.metadata.welcome?.sentAt) {
        await openConversationIfNew();
        pinShortcuts().catch((e) => console.warn("[wa-linked] shortcuts:", e.message));
      }
...
module.exports = { setup, sendLinkedMessage, sendLinkedDocument, sendLinkedVoice, sendLinkedLocation, closeForShutdown };
