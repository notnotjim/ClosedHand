...
// Every readable message, any chat, becomes recall material: cached and
// vector-indexed so "what did Seval say about customs" works across chats.
...
async function ingestMessages(userId, entries, chatNames = {}) {
  ...
  try {
    const { indexItems } = require("../services/usi");
    await indexItems(userId, "whatsapp", "message", items.map((i) => ({
      external_id: i.external_id,
      text: `[WhatsApp ${i.chat_name}] ${i.sender}: ${i.text} (${i.date.substring(0, 10)})`,
      metadata: { chat: i.chat, chat_name: i.chat_name, sender: i.sender, date: i.date },
    })));
  } catch (e) {
    console.log(`[wa-linked] index skipped: ${e.message}`);
  }
  return items.length;
}
