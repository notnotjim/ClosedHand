    ...
    sock.ev.on("messages.upsert", async ({ messages, type }) => {
      for (const msg of messages || []) {
        try {
          ...
          if (isSelfJid(jid) && type === "notify") {
            // A reply gets its quoted referent prefixed, so the model reads
            // "cancel this" against the message it was actually attached to
            // rather than against whatever was said last.
            ...
            const engineText = quoted
              ...
              : text;
            console.log(`[wa-linked] self-chat message: "${text.substring(0, 60)}"${quoted ? " (a reply)" : ""}`);
            await handleSelfChatMessage(engineText);
          }
        } catch (e) {
