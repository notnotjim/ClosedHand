...
async function getDashboardOverview({ db, userId, platform, getUrl, now = Date.now() }) {
      ...
      items = items.flatMap(row => {
        try {
          ...
          return [{
            ...
            status: FlightTime.status(f, now).label,
            past: FlightTime.status(f, now).past,
            live_tracking_available: !!process.env.FLIGHTAWARE_API_KEY,
          }];
        } catch (_) { unreadable++; return []; }
      }).sort((a, b) => new Date(a.departure.dateTime) - new Date(b.departure.dateTime));
    }
    if (name === "bookings") items = items.filter(b => !b.ends_at || Date.parse(b.ends_at) >= now).map(b => ({
  ...
  overview.link_note = overview.dashboard_url
    ...
    : overview.unavailable.includes("dashboard_url")
      ...
      : "Phone access has no configured address. Enable Your phone in dashboard Settings on the computer running ClosedHand, then ask again.";
