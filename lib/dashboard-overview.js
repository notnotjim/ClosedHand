...
function localPoint(point = {}, actualTime) {
...
async function getDashboardOverview({ db, userId, platform, getUrl, now = Date.now() }) {
  ...
  const overview = { checked_at: new Date(now).toISOString(), dashboard_url: null, sections: {}, unavailable: [] };
  settled.forEach((result, index) => {
    const name = names[index];
    if (result.status === "rejected" || (name !== "dashboard_url" && result.value?.error)) {
      overview.unavailable.push(name);
      if (name !== "dashboard_url") overview.sections[name] = { available: false, count: null, items: null };
      return;
    }
    if (name === "dashboard_url") { overview.dashboard_url = result.value || null; return; }
      ...
      items = items.flatMap(row => {
        try {
          ...
          return [{
            ...
            departure: localPoint(f.departure, f.liveStatus?.departureTime),
            arrival: localPoint(f.arrival, f.liveStatus?.arrivalTime),
