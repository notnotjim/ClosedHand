...
async function handleOnboardingMessage(userId, chatId, text) {
  ...
  switch (step) {
    ...
    case "name_bot": {
      ...
      if (split.rest) await rememberPending(split.rest);
      ...
      saveStore();
      await sendText(chatId, nameConfirm);
      return;
    }
    ...
    case "greet_user": {
      ...
      saveStore();
      await sendText(chatId, locationAsk);
      return;
    }
...
function privacyLine() {
  if (isSelfHost()) {
    return "What I remember is stored on the computer or server running ClosedHand. You can review or delete it in your dashboard. If you use a hosted LLM, your requests and relevant context are sent to that provider.";
  }
  return "What I remember is stored in ClosedHand's hosted service. You can review or delete it in your dashboard. Your requests and relevant context are sent to the LLM provider used to answer you.";
}
