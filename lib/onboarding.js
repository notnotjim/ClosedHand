...
// "Richard. And btw, can you find that story about..." is an answer and a
...
// The question bundled with the first answer, run through the engine at the
// end of onboarding, inside the same turn the platform already holds.
async function answerPending(userId, chatId) {
  ...
  try {
    ...
    console.error(`[Onboarding] bundled question failed: ${e.message}`);
  }
}
...
- profile-name: their name as they sign it
...
async function scanHighlight(userId) {
  try {
    const notes = Object.entries(ctx.store.facts || {})
      .filter(([k]) => !k.startsWith("_") && k !== "profile-name")
      .slice(0, 12);
    if (!notes.length) return "";
    const { client, model } = getInternalClient(userId);
    const prompt = `You are ClosedHand, a personal assistant that has just read a new user's recent email and calendar for the first time. From these notes, write ONE sentence (under 30 words, first person, no greeting, no question) that shows you have already done that by mentioning one or two specific things. Plain and friendly, not salesy. No names of people other than the user.

NOTES:
${notes.map(([k, v]) => `${k}: ${v}`).join("\n")}`;
    const res = await client.messages.create({ model, max_tokens: 90, messages: [{ role: "user", content: prompt }] });
    const line = (responseText(res) || "").trim().split("\n")[0];
    return line && line.length < 220 ? line : "I've already had a look through your inbox and calendar.";
  } catch (_) {
    return "I've already had a look through your inbox and calendar.";
  }
}
...
async function handleOnboardingMessage(userId, chatId, text) {
  ...
  const rawProfileName = ctx.activeUserStore?.profile?.display_name
    ...
    || null;
  // The profile's own name, or the one the inbox scan found.
  const profileName = firstName(rawProfileName) || firstName(ctx.store.facts["profile-name"]);

  switch (step) {
    case null:
    case undefined: {
      // First contact. The scan usually ran when Google was connected; if
      // not, it starts now and the next turns benefit.
      startBackgroundScan(userId);

      const highlight = await scanHighlight(userId);
      const greeting = (profileName ? `Hey ${profileName}, ` : "Hey, ")
        + "ClosedHand here. I'll be living in this chat from now on: ask me things, send me stuff, and I'll keep an eye on your mail and calendar in between. "
        + (highlight ? highlight + " " : "")
        + "First things first, what would you like to call me? Any name you like.";
    ...
    case "name_bot": {
      const split = splitLeadingAnswer(text);
      if (isNotAnAnswer(split.answer)) {
        const again = "Hi! Before anything else, I need a name. Anything you like, one or two words, and then we're off.";
        conversation.push({ role: "user", content: text });
        conversation.push({ role: "assistant", content: again });
        saveStore();
        sendText(chatId, again);
        return;
      }
      // User gave a bot name
      const botName = split.answer.split(/\s+/).slice(0, 3).join(" "); // Cap at 3 words
      await saveProfileSetting("bot_name", botName);
      if (split.rest) await saveProfileSetting("onboarding_pending", split.rest);

      const nameConfirm = (profileName
        ? `${botName}. I like it. And you're ${profileName}, right?`
        : `${botName}. I like it. And what should I call you?`)
        + (split.rest ? " I've got your question too, coming right after this." : "");
    ...
    case "greet_user": {
      ...
      let preferredName;
      const saidYes = ["yes", "yeah", "yep", "yea", "that's right", "correct", "ye"].includes(lower);
      if (!saidYes && isNotAnAnswer(text)) {
        const again = profileName ? `Are you ${profileName}? A yes will do, or tell me your first name.` : "And your first name?";
        conversation.push({ role: "user", content: text });
        conversation.push({ role: "assistant", content: again });
        saveStore();
        sendText(chatId, again);
    ...
    case "ask_location": {
      if (/\?\s*$/.test(text.trim())) {
        const again = "Where you are based. A city is enough, or say skip.";
        conversation.push({ role: "user", content: text });
        conversation.push({ role: "assistant", content: again });
        saveStore();
        sendText(chatId, again);
      ...
      if (!isSkip) {
        try {
          ...
          const geoRes = await fetch(geoUrl, {
            headers: { "User-Agent": "ClosedHand/1.0 (contact@closedhand.com)" },
          });
...
// Self-host runs on the person's own machine against their own database;
// hosted runs on ours. The sign-off says what is true for the one it is on.
function isSelfHost() {
  return process.env.DB_DRIVER === "pg" || (!!process.env.DATABASE_URL && !process.env.SUPABASE_URL);
}
function privacyLine() {
  if (isSelfHost()) {
    return "Everything I keep lives on this machine, in your dashboard, yours to read or delete. Nothing reaches us, ever. The only thing that leaves is what you say to the model you picked, so it can answer you.";
  }
  return "Your messages and data are encrypted, stay on your server, can't be read by anyone (including us), never used for training, and you can wipe everything whenever you want.";
}
...
function generateOnboardingSignOff(userId, chatId) {
  ...
  if (hasNotes) {
    ...
    msg += `\n\nThis chat is one conversation. Send /new to start a fresh one, /threads to go back to an old one.`;

    msg += `\n\n${privacyLine()} Just ask me stuff. I'm here.`;
    return msg;
  }
...
module.exports = { WELCOME_MESSAGE, handleOnboardingMessage, generatePlatformWelcome, startBackgroundScan, watchForFirstGoogle };
