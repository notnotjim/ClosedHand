...
function getContextWindow(provider) {
  if (Number.isFinite(provider?.contextWindow) && provider.contextWindow >= 4096) return provider.contextWindow;
  ...
  if (Array.isArray(content)) {
    ...
    for (const block of content) {
      if (block.type === "provider_state") {
        total += estimateStringTokens(JSON.stringify(block.value || block.geminiPart || {}));
