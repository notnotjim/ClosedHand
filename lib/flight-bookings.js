...
function reconcileFlights(facts, parsed, emails, now = Date.now()) {
  ...
  const initialKeys = new Set(records.keys());
  ...
  for (const { f, email } of entries) {
    ...
    if (!repeated && previous && !f.pending) {
      for (const end of ['departure', 'arrival']) {
        const from = previous[end], to = next[end];
        if (dateMs(from?.dateTime) && dateMs(to?.dateTime) && dateMs(from.dateTime) !== dateMs(to.dateTime)) {
          scheduleChanges[end] = { from, to };
        }
      }
    }
    ...
    if (replacement) {
      ...
      if (oldMatches.length <= 1) {
        ...
        // Agencies can expose both a customer reference and an airline PNR.
        // Different references are compatible only when this explicit change
        // email also contains the old one; unrelated bookings remain separate.
        const oldReference = compact(oldRecord?.confirmationCode);
        const refConflict = oldReference && next.confirmationCode && oldReference !== compact(next.confirmationCode)
          && !(oldReference.length >= 4 && text.includes(oldReference));
    ...
    const earlier = announced.get(key);
    if (!previous || replacedNumber || Object.keys(scheduleChanges).length || earlier) {
      // A batch can contain the original confirmation and several later changes.
      // Announce only the final itinerary, retaining the first known old clock.
      const changes = earlier ? { ...(earlier.scheduleChanges || {}) } : scheduleChanges;
      if (earlier?.scheduleChanges) for (const [end, change] of Object.entries(scheduleChanges)) {
        changes[end] = { from: changes[end]?.from || change.from, to: change.to };
      }
      for (const [end, change] of Object.entries(changes)) {
        change.to = next[end];
        if (dateMs(change.from.dateTime) === dateMs(change.to?.dateTime)) delete changes[end];
      }
      announced.set(key, { key, ...next,
        ...(Object.keys(changes).length ? { scheduleChanges: changes } : {}),
        ...((replacedNumber || earlier?.replacesFlightNumber) ? { replacesFlightNumber: replacedNumber || earlier.replacesFlightNumber } : {}) });
    }
  }
  ...
  return { patches: [...patches], flights: [...announced.values()].filter(f => !records.get(f.key)?.supersededBy
    && (!initialKeys.has(f.key) || f.replacesFlightNumber || Object.keys(f.scheduleChanges || {}).length)) };
}
