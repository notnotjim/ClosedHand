...
async function announceNewFlights(userId, userStore, newFlights, chatLinks) {
  ...
  const WEBAPP_URL = await require("./config").dashboardBase();
  for (const flight of newFlights) {
    const when = new Date(flight.departure?.dateTime).toLocaleDateString("en-GB");
    const msg = `Found your ${flight.flightNumber} to ${flight.arrival?.airport || "destination"} on ${when}. ${LIVE ? "I'll keep you posted." : "It's on your dashboard under Upcoming."}`;
    for (const link of targets) {
      try {
        if (link.platform === "telegram" && ctx.bot && WEBAPP_URL) {
