...
const _lastMailScan = new Map(); // userId -> ms of the last mail scan for new flights

// Tells the user about flights just found, on the apps they chose for
// proactive messages, and starts the live loop when there is a key to poll.
async function announceNewFlights(userId, userStore, newFlights, chatLinks) {
  if (!chatLinks) {
    const { data } = await supabase.from("chat_links").select("platform, platform_user_id").eq("user_id", userId).not("platform_user_id", "is", null);
    chatLinks = data || [];
  }
  if (LIVE && !flightIntervals[userId]) startFlightCheckForUser(userId);
  if (!chatLinks.length) return;
  const targets = await deliveryTargets(userId, userStore, chatLinks);
  for (const flight of newFlights) {
    let current;
    try { current = JSON.parse(_getNoteValue(userStore.notes[flight.key])); } catch (_) {}
    if (current?.supersededBy || flight.supersededBy) continue;
    const when = FlightTime.dateLabel(flight.departure?.dateTime, flight.departure);
    const changes = Object.entries(flight.scheduleChanges || {}).map(([end, change]) => {
      const from = FlightTime.time(change.from.dateTime, change.from);
      const to = FlightTime.time(change.to.dateTime, change.to);
      const oldDate = FlightTime.dateKey(change.from.dateTime, change.from);
      const newDate = FlightTime.dateKey(change.to.dateTime, change.to);
      const was = oldDate === newDate ? from : `${from} on ${FlightTime.dateLabel(change.from.dateTime, change.from)}`;
      return `${end === "departure" ? "Departs" : "Arrives"} ${to} on ${FlightTime.dateLabel(change.to.dateTime, change.to)} (was ${was}, ${change.to.airport || end} local time).`;
    });
    const msg = flight.replacesFlightNumber
      ? `Your ${flight.replacesFlightNumber} has been replaced by ${flight.flightNumber} on ${when}, departing ${FlightTime.time(flight.departure?.dateTime, flight.departure)} (departure airport local time). I've updated your itinerary.`
      : changes.length ? `Your ${flight.flightNumber} schedule has changed. ${changes.join(" ")} I've updated your itinerary.`
      : `Found your ${flight.flightNumber} to ${flight.arrival?.airport || "destination"} on ${when}. ${LIVE ? "I'll keep you posted." : "It's on your dashboard under Upcoming."}`;
    let delivered = false;
    for (const link of targets) {
      try {
        // An unavailable dashboard must not suppress a time-sensitive alert.
        const url = await dashboardUrl(link.platform, "schedules").catch(() => null);
        if (link.platform === "telegram" && ctx.bot && url) {
          ...
          const receipt = await sendToPlatform(link.platform, link.platform_user_id, msg + (url ? `\n\nView flight schedules: ${url}` : ""));
          if (receipt?.error || receipt?.isError) throw new Error("Delivery failed");
        }
        delivered = true;
      } catch {
        console.error(`Flights: announcement delivery failed for ${link.platform}; ${flight.flightNumber}`);
      }
    }
    if (delivered) {
      getConversation(userId).push({ role: "assistant", content: `[Flight update] ${msg}` });
      saveStore();
    }
  }
}

// Live status (gates, delays, landing) needs a FlightAware key. Finding the
// flights in the first place does not, and used to be switched off with it:
// an install without the key never read a single confirmation.
const LIVE = !!process.env.FLIGHTAWARE_API_KEY;

function startFlightTracking() {
  if (!LIVE) console.log("Flights: no FLIGHTAWARE_API_KEY, so flights are found from mail and shown without live status.");
    ...
    for (const user of users) {
      ...
      try {
        ...
        for (const key of flightKeys) {
          ...
          if (!flight || flight.landed || flight.supersededBy) continue;
          ...
          if (!sched.error) {
            ...
            if (retimed) {
              const msg = `Schedule change: ${flight.flightNumber} now departs ${FlightTime.time(sched.scheduledOut, flight.departure)} on ${FlightTime.dateLabel(sched.scheduledOut, flight.departure)} (was ${FlightTime.time(oldDep, flight.departure)}, departure airport local time).`;
        ...
        if (!recentFlightEmail || recentFlightEmail.length === 0) {
          ...
          upcomingNoted = Object.keys(store.notes || {}).some(k => {
            ...
            try {
              ...
              if (f.landed || f.supersededBy) return false;
              return FlightTime.shouldTrack(f, Date.now(), 72);
            } catch { return false; }
          });
        }
  ...
  try {
    ...
    const activeFlightKeys = Object.keys(ctx.store.facts).filter(k => {
      ...
      try {
        ...
        if (f.landed || f.supersededBy) return false;
        return FlightTime.shouldTrack(f, now);
      } catch { return false; }
    });

    // A scan for new bookings runs when nothing is being tracked, and every
    // six hours regardless, so a flight booked mid-trip is not missed. Mail
    // that arrives between passes is read on arrival (lib/mail-noticing.js).
    const due = Date.now() - (_lastMailScan.get(userId) || 0) > DISCOVERY_INTERVAL;
    if (activeFlightKeys.length === 0 || due) {
      _lastMailScan.set(userId, Date.now());
      ...
      const newFlights = [...new Map([...calFlights, ...emailFlights].map(f => [f.key, f])).values()];
      if (newFlights.length > 0) await announceNewFlights(userId, userStore, newFlights, chatLinks);
      if (activeFlightKeys.length === 0) {
        if (newFlights.length === 0) stopFlightCheckForUser(userId);
        saveStore();
        cleanupUserContext();
        return;
      }
...
module.exports = { startFlightTracking, startFlightCheckForUser, stopFlightCheckForUser, announceNewFlights };
