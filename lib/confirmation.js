...
async function handleConfirmation(userId, chatId, response) {
  ...
  const isYes = response.toLowerCase().match(/^(yes|y|yep|yeah|go|confirm|do it|ok|send it|always)$/);
  ...
  if (isYes && pending.outbound && pending.outbound.host && /^always$/i.test(response.trim())) {
  ...
  try {
    ...
    // carry on with the cloud browser after a confirmation as it could before.
...
module.exports = { ACTIONS_NEEDING_CONFIRMATION, handleConfirmation, dropPending };
