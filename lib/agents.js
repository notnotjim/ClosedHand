...
async function runMapSub(userId, subGoal, platform, chatId, model) {
  ...
  for (let i = 0; i < MAP_SUB_MAX_ITERATIONS; i++) {
    const response = await createAgentResponse(client, { model, max_tokens: 4096, system: systemPrompt, messages, tools }, userStore?.profile?.settings?.llm_provider);
  ...
  try {
    ...
    const fin = await createAgentResponse(client, { model, max_tokens: 4096, system: systemPrompt, messages, tools }, userStore?.profile?.settings?.llm_provider);
    let text = "";
...
async function startAgent(userId, platform, chatId, goal, successCriteria) {
  ...
  ctx.runWithInheritedContext(() => runAgent(task.id, userId, platform, chatId, goal, model, criteria)).catch((err) => {
    ...
    sendToPlatform(platform, chatId, `Agent failed: ${err.message}\n\nThat looks like a bug on my side. Send /bug to flag it.`).catch(() => {});
  });
...
async function runAgent(taskId, userId, platform, chatId, goal, model, successCriteria, resumeState = null) {
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    if (Date.now() - startTime > AGENT_TIMEOUT_MS) {
      ...
      await sendToPlatform(platform, chatId, "I ran out of time on that one before finishing. Want me to pick it back up?\n\n" + await require("./dashboard-links").agentLinkNotice(platform, "What I managed so far is on your dashboard"));
      return;
    }
  ...
  try {
    ...
    const summary = await createAgentResponse(wrapUpClient, {
      model,
      max_tokens: 1024,
