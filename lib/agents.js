...
function describeAgentProgress(lastTool, totalTools, allTools, model, elapsedMs = 0, lastToolInput = null) {
  const toolActions = {
    ...
    onedrive_search: "Looking through your OneDrive",
...
function buildAgentSystemPrompt(goal, userStore, platform = "web") {
...
4. DELIVER: Answer the user's question directly in chat. Match the length to what they asked, not to how much work you did. A few sentences or a short list is usually enough. Preserve the facts, sources and caveats needed to use the answer. Summarise sources accurately; quote their exact wording only when requested or when the wording matters. Do not turn an ordinary answer into a standalone report or create a file, executive summary or recommendations section merely because an agent was used. Create a full report, document, spreadsheet or other separate deliverable when the user requests one or the task explicitly needs that format. In that case, make the deliverable complete and give a brief useful answer in chat.
...
- A source that refuses you (429, rate limiting, timeouts) will not change its mind in the next minute. Do not retry, wait, sleep or poll for it. Take what you have, answer, and say what is blocked. Ten minutes fighting a limit is worse than a partial answer in one.
- Batch related sandbox_browse actions to avoid repeated browser connections. Observe the page before choosing dependent actions; do not guess unseen buttons or results.
- Do not narrate tool calls in the delivered answer. Report useful findings, failures and any action needed from the user.
...
Persistence is for things that are failing, not for things that are absent. If one tool errors, try another. If a service is down, find another route. But do not wander into unrelated tools to look busy: hunting for a Drive file with a web search or the sandbox shell is not thoroughness, it is noise.`;
  ...
  return prompt + require("./response-presentation").responsePresentation(platform);
}
...
async function runMapSub(userId, subGoal, platform, chatId, model) {
  ...
  for (let i = 0; i < MAP_SUB_MAX_ITERATIONS; i++) {
    const response = await createAgentResponse(client, { model, effort: userStore?.profile?.settings?.agent_worker_tier === "fast" ? "fast" : "default", max_tokens: 4096, system: systemPrompt, messages, tools: toolScope.definitions(messages) }, userStore?.profile?.settings?.llm_provider);
...
function sendDeliveryKey(name, input = {}) {
  switch (name) {
    ...
    case "onedrive_send_file": return input.file_id ? "odrv:" + input.file_id : null;
  ...
  try {
    return await ctx.runWithInheritedContext(async () => {
      ...
      return withTaskRun({ userId, taskId, kind: "agent", leaseOwner: task.lease_owner, budget, signal: controller.signal }, async () => {
        ...
        const runtime = { ...(task.runtime || {}), modelTier: prepared.tier || task.runtime?.modelTier || "default", prepared: true, budget: budgetSnapshot() };
...
async function runAgentLoop(taskId, userId, platform, chatId, goal, model, successCriteria, resumeState = null) {
  ...
  const systemPrompt = buildAgentSystemPrompt(goal, userStore, platform) + toolScope.catalog;
  ...
  const stuckStreak = { sign: null, count: 0 };
  let timeNotes = 0;
  while (iteration < MAX_ITERATIONS) {
    ...
    const response = await createAgentResponse(agentClient, {
      model,
      effort: currentTask?.runtime?.modelTier || "default",
      max_tokens: 4096,
      system: systemPrompt,
      messages,
    ...
    if (pendingSend) {
      ...
      const question = pendingSend._spendCard ? pendingSend._spendCard : describeAgentSend(pendingSend) + claimsNote + draftNote;
      ...
      ctx.pendingConfirmations[userId] = {
        ...
        asked: { platform, at: Date.now(), text: String(question) },
      ...
      await sendToPlatform(platform, chatId, question);
      console.log(`Agent ${taskId} paused for send confirmation (${pendingSend.name}, draft=${draftId ? "yes" : "no"})`);
      return;
    }

    // The same failure, iteration after iteration, is not progress: after
    // three the model is told to stop that route and report; after six, to
    // answer now with what it has.
    const stuck = require("./stuck-tools").stuckNote(toolResults, stuckStreak);
    if (stuck) { toolResults.push({ type: "text", text: stuck }); console.log(`Agent ${taskId} stuck: ${stuckStreak.sign} x${stuckStreak.count}`); }
    // Nothing told the agent how much time it had. Past the halfway mark it
    // hears the clock once; near the end it is told to answer with what it has.
    const usedShare = (Date.now() - startTime) / AGENT_TIMEOUT_MS;
    const minutesLeft = Math.max(0, Math.round((AGENT_TIMEOUT_MS - (Date.now() - startTime)) / 60000));
    if (usedShare >= 0.85 && timeNotes < 2) {
      timeNotes = 2;
      toolResults.push({ type: "text", text: `Note from the system: about ${minutesLeft} minute${minutesLeft === 1 ? "" : "s"} of your time allowance remain. Make no more tool calls. Write the final answer now from what you have, and say what was not checked.` });
    } else if (usedShare >= 0.6 && timeNotes < 1) {
      timeNotes = 1;
      toolResults.push({ type: "text", text: `Note from the system: about ${minutesLeft} minutes of your time allowance remain. If the evidence already answers the question, write the answer now; spend what is left only on something that would change it, not on confirming it another way.` });
    }
    messages.push({ role: "user", content: toolResults });
...
async function resumeAgents() {
  const { supabase } = require("./db");
  const { data, error } = await supabase.from("agent_tasks").select("id,user_id,runtime").eq("status", "awaiting_confirmation");
  if (error) throw error;
  ctx.pendingConfirmations ||= {};
  const { isLapsed } = require("./confirmation-lifetime");
  for (const row of data || []) {
    const pending = row.runtime?.confirmation;
    // Only a question still within its day comes back to life after a restart.
    if (!pending || isLapsed(pending)) { await dropConfirmation({ isAgent: true, taskId: row.id }, pending ? "lapsed" : "unresumable"); continue; }
    ctx.pendingConfirmations[row.user_id] = pending;
  }
  await processPendingTasks();
}

// The question a paused agent asked will not be answered: record that on the
// task so it leaves the waiting list and is never revived.
async function dropConfirmation(pending, reason) {
  if (!pending?.isAgent || !pending.taskId) return;
  const saved = await getTask(pending.taskId).catch(() => null);
  if (!saved || saved.status !== "awaiting_confirmation") return;
  const { droppedNote } = require("./confirmation-lifetime");
  await updateTask(pending.taskId, { status: "cancelled", error: droppedNote(reason), completed_at: new Date().toISOString() });
  console.log(`Agent ${pending.taskId} dropped its unanswered question (${reason})`);
}
...
module.exports = { startAgent, cancelAgent, resumeAgents, processPendingTasks, resumeAgentAfterConfirmation, dropConfirmation };
