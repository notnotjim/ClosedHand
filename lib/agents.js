...
async function runAgent(taskId, userId, platform, chatId, goal, model, successCriteria, resumeState = null) {
  ...
  while (iteration < MAX_ITERATIONS) {
    ...
    if (response.stop_reason !== "tool_use") {
      ...
      if (platform !== "dashboard" && (reportText || "").length > 1200) {
        try {
          ...
          if (digest) chatBody = digest + `\n\nFull report: ${require("./config").dashboardBase()}/dashboard#agents`;
    ...
    if (pendingSend) {
      ...
      await sendToPlatform(platform, chatId, pendingSend._spendCard ? pendingSend._spendCard : describeAgentSend(pendingSend) + draftNote);
