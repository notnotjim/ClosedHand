...
const _deferred = new Map();     // userId -> the one follow-up still to be delivered
function scheduleDeferredWrapUp(info) {
  const { acquireUserMutex } = require("./user-mutex");
  // Only the latest interrupted turn gets a follow-up; an earlier one still
  // waiting is dropped, its messages are in the conversation for the newer
  // answer to use.
  const prev = _deferred.get(info.userId);
  if (prev) prev.cancelled = true;
  const ticket = { cancelled: false };
  _deferred.set(info.userId, ticket);
  setTimeout(() => {
    acquireUserMutex(info.userId, async () => {
      if (ticket.cancelled) return;
      if (_deferred.get(info.userId) === ticket) _deferred.delete(info.userId);
      try {
