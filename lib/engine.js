...
`;
  ...
  prompt += `INDEX FIRST: To find files, documents, or content by topic or meaning, ALWAYS use semantic_search / search_cache first - the pre-built index is faster, cheaper, and matches paraphrases. Only fall back to live provider APIs (drive tools, sandbox_gateway) when the index misses, the data is minutes old, or you need to fetch the full file the index located. Do not iterate keyword guesses against provider APIs when an indexed search exists.
...
async function ask(userId, userMessage, fileData = null, chatId = null, deepThink = false, opts = {}) {
  ...
  try {
    ...
    while (maxIterations > 0) {
      ...
      // Unified passive recall: searches all data_vectors + rag_chunks, reranks combined results
