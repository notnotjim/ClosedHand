...
function buildVolatileSystemTail(lastUserMessage = "", contextInjection = "") {
  ...
  if (!timeBlock) {
    try {
      timeBlock = "Today is " + now.toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric", timeZone: tz }) + ". Current time where the user is: " + now.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", timeZone: tz }) + " (" + tz + "). Use " + tz + " for ordinary event times; keep flight and booking times in the airport or property's local timezone unless the user asks for a conversion.";
    } catch (e) {
  ...
  return "\n\n" + timeBlock + buildTravelTimezoneBlock(tz) + accountsBlock + skillsBlock + mattersBlock + (contextInjection || "");
}
...
When asked to show/open/link the dashboard or Agents page, or describe its upcoming schedule, call get_dashboard_overview. Share its exact dashboard_url, even if there are no running agents. Never infer that phone access is unavailable from an old localhost link in the conversation. Only explain enabling phone access if the fresh tool result says no address is configured. For an overview, include flights and hotel/other bookings alongside reminders and agents; list_schedules alone does not describe the page. Report a failed section as unavailable, not empty. Use the supplied travel-local clocks without converting them to the user's current location. Do not start a background agent just to get this overview.\n`;
