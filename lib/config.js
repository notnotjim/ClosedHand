...
// The address a link to the dashboard should use. With "Your phone" on, the
// tunnel address (the webapp keeps PHONE_ACCESS_URL current), so a link tapped
// on a phone opens; otherwise the configured base, localhost on this machine.
function dashboardBase() {
  const phone = getConfCached("PHONE_ACCESS_URL");
  if (phone && /^https:\/\//.test(phone)) return phone.replace(/\/$/, "");
