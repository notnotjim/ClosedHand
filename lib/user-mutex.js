...
function acquireUserMutex(userId, fn) {
  ...
  return new Promise((resolve, reject) => {
    queues[userId] = queues[userId].then(async () => {
      ...
      let done = false;
      let timer = null;
      // Time the machine spent asleep is not a stall: when this watchdog's
      // own tick arrives late by more than a sleep gap, progress is reset to
      // now rather than the turn being killed for the lid having been closed.
      let lastTick = Date.now();
      const idleWatch = new Promise((_, rej) => {
        timer = setInterval(() => {
          if (done) { clearInterval(timer); return; }
          const now = Date.now();
          if (now - lastTick > require("./sleep-watch").SLEEP_GAP_MS) progress[userId] = now;
          lastTick = now;
          if (now - (progress[userId] || 0) > IDLE_TIMEOUT_MS) {
            clearInterval(timer);
