      ...
      for (const block of response.content) {
        if (block.type === "tool_use") {
          ...
          if (isInternalTool(block.name)) {
            ...
            result = spendGuard.spendIntent(block.name, schedInput)
              ? { error: "Not done. A scheduled task never pays for anything. Tell the user what needs paying and how much; they will do it in chat." }
              : await handleInternalTool(block.name, schedInput);
