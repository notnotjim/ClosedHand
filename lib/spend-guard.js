...
// Page context is weaker evidence than an explicit Pay action. Inspect the
// path, not the hostname/query, and distinguish payment lookups from checkout.
function isCheckoutPage(url, title = "") {
  let path;
  try { path = decodeURIComponent(new URL(url).pathname); } catch { path = ""; }
  const lookup = /\b(?:payments?|transactions?)[\s_/-]*(?:lookup|search|history|status|receipt)\b|\b(?:lookup|search|history|status|receipt)[\s_/-]*(?:payments?|transactions?)\b/i;
  const pathCheckout = CHECKOUT_URL.test(path) && !lookup.test(path);
  const titleCheckout = CHECKOUT_URL.test(title) && !lookup.test(title);
  return /(?:^|\/)(?:checkout|purchase)(?:\/|$)/i.test(path) || pathCheckout || titleCheckout;
}
...
function spendIntent(toolName, input) {
  ...
  const page = lastPage(input._userId);
  const onCheckout = !!(page && isCheckoutPage(page.url, page.title));
  ...
  if (toolName === "sandbox_browse") {
    ...
    if (a === "click" || a === "press" || (a === "fill" && input.submit)) {
      ...
      if ((onCheckout || (at && isCheckoutPage(at))) && (a === "press" ? /enter/i.test(textOf(input.key)) : /submit|button|btn|pay|order|confirm|continue|complete|book/i.test(sel))) return { kind: "click", why: "a submit on a checkout page", at };
    }
    if (a === "eval_js") {
    ...
    if (a === "batch" && Array.isArray(input.steps)) {
      ...
      let where = at;
      let sequenceCheckout = at ? isCheckoutPage(at) : onCheckout;
      for (const st of input.steps) {
        if (!st || typeof st !== "object") continue;
        if (st.url) { where = String(st.url); sequenceCheckout = isCheckoutPage(where); }
        const s = textOf(st);
        ...
        if (/"action":"(click|press)"|"submit":true/.test(JSON.stringify(st)) && sequenceCheckout) return { kind: "click", why: "a submit on a checkout page within this sequence", at: where };
...
// Spent since a moment, in the limit currency. A row whose currency cannot
// be converted counts at face value, which errs towards asking.
async function spentSince(userId, sinceIso, currency) {
  ...
  for (const r of data || []) {
    ...
    const converted = currency && r.currency ? await require("./fx").convert(v, r.currency, currency) : v;
    sum += converted == null ? v : converted;
  }
  return sum;
}
...
async function assess({ userId, userStore, toolName, source, intent, page }) {
  ...
  // Compared in the limit's currency. When the shop's currency is known but
  // no rate can be had, the limits cannot be judged, so the purchase is put
  // to the person instead of being refused or allowed on the wrong number.
  const rawAmount = total ? total.value : null;
  let amount = rawAmount, unconverted = false;
  if (rawAmount != null && limits.currency && total.currency && total.currency !== limits.currency) {
    const converted = await require("./fx").convert(rawAmount, total.currency, limits.currency);
    if (converted == null) unconverted = true; else amount = converted;
  }
  if (unconverted) { limits.per_purchase = null; limits.per_day = null; limits.per_month = null; limits.auto_under = null; limits.always_ask = true; }
  ...
  const base = {
    ...
    merchant, amount: rawAmount, currency: total ? total.currency : null, amount_text: total ? total.text : null, tool: toolName, source,
  ...
  if (limits.per_day != null) {
    const day = await spentSince(userId, new Date(Date.now() - 24 * 3600000).toISOString(), limits.currency);
  ...
  if (limits.per_month != null) {
    const month = await spentSince(userId, new Date(Date.now() - 30 * 86400000).toISOString(), limits.currency);
