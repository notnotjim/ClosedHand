...
async function holdForUser({ userId, platform, chatId, toolName, toolInput, verdict, source, label }) {
  ...
  ctx.pendingConfirmations[userId] = {
    ...
    toolUseId,
    spend: { ledgerId: verdict.ledgerId },
    heldFrom: source,
    messages: [
  ...
  const intro = `${label || "A background task"} wants to make a purchase. ${verdict.card}`;
  try {
