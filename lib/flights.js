...
// PDF text comes from lib/mail-attachments (shared with the bookings scan).
const { readAttachmentText: pdfAttachmentText } = require("./mail-attachments");

// opts.emails: scan just these cached emails (the ones a sync brought in)
// instead of searching the whole cache.
async function scanEmailsForFlights(userId, opts = {}) {
  ...
  // Collect email bodies for batch extraction. A forwarded confirmation
  // often says only "itinerary attached": then the attachment is the body.
  const { bodyForScan, attachmentTextFor } = require("./mail-attachments");
  const flightEmails = emails.filter(e => e.id && (e.summary || e.body || (e.attachments && e.attachments.length)));
  // Search results may contain only a preview. Read the full cached email so
  // an "original flight" block cannot hide the "new flight" below it.
  const previews = flightEmails.filter(e => !e.body).map(e => e.id);
  if (previews.length) {
    const { supabase } = require("../user-store");
    const { data } = await supabase.from("data_cache").select("external_id, data")
      .eq("user_id", userId).eq("type", "email").in("external_id", previews);
    for (const email of flightEmails) {
      const full = (data || []).find(row => row.external_id === email.id)?.data;
      if (full?.body) email.body = full.body;
      if (!email.attachments && full?.attachments) email.attachments = full.attachments;
      if (!email.date && full?.date) email.date = full.date;
    }
  }
  const emailBodies = [];
  for (let idx = 0; idx < flightEmails.length; idx++) {
    const e = flightEmails[idx];
    emailBodies.push({ emailIndex: idx, id: e.id, subject: e.subject || "", date: e.date || e.received_at || "", body: await bodyForScan(userId, { ...e, summary: e.body || e.summary }, 12000) });
  }

  if (emailBodies.length === 0) return [];

  // A delay email can name the flight but leave arrival and terminal details
  // only in the itinerary. Read its attachments too, once through the mail cache.
  // The cache helper limits this to two readable files, 5 MB each, 6,000 chars.
  for (const eb of emailBodies) {
    const src = flightEmails[eb.emailIndex];
    if (!src.attachments?.length || /\[Attachment /i.test(eb.body)) continue;
    const extra = await attachmentTextFor(userId, src);
    if (extra) eb.body += `\n\n${extra}`;
  }
  ...
  try {
    ...
    const extraction = await llm.messages.create({
      model: fastModel,
      max_tokens: 4096,
      messages: [{ role: "user", content: `Extract flight booking details from these ${emailBodies.length} emails. For each email that contains flight details, return a JSON object. Return a JSON array of results. Skip emails with no flight details. Do NOT invent or guess. Return ONLY valid JSON, no markdown, no code fences.\n\nFormat per flight: { "emailIndex": 0, "airline": "...", "flightNumber": "...", "departure": { "airport": "IATA code", "dateTime": "ISO 8601 WITH the airport's UTC offset", "tz": "airport IANA timezone" }, "arrival": { "airport": "IATA code", "dateTime": "ISO 8601 WITH the airport's UTC offset", "tz": "airport IANA timezone" }, "confirmationCode": "...", "passengerName": "...", "pending": false }\n\nIMPORTANT:\n- "pending" is true when the email says the booking is not yet confirmed (awaiting or pending airline confirmation, on request, waitlisted); false for a confirmed booking, e-ticket or itinerary.\n- The current year is ${currentYear}. If an email does not state a year, use ${currentYear}.\n- Airport fields must be 3-letter IATA codes (LHR, JFK, PVG), not city names.
- flightNumber and airline must be COPIED from the email text. Many booking emails (agencies like Mytrip) state routes and times but no flight number: for those return "flightNumber": null. NEVER supply a plausible flight number the email does not contain; a wrong one corrupts live tracking.\n- Times in emails are LOCAL to the airport. dateTime MUST carry the airport's UTC offset (e.g. a 01:25 departure from Singapore is "2026-07-28T01:25:00+08:00", never "2026-07-28T01:25:00" or with Z). Work out the offset from the airport.\n- If one email has multiple flights (outbound + return), return separate objects with the same emailIndex.\n- For a changed or rebooked itinerary, return ONLY the new flight as active. Include "replaces": { "flightNumber": "old number", "departure": { "airport": "old origin", "dateTime": "old ISO time with offset", "tz": "IANA zone" }, "arrival": { "airport": "old destination", "dateTime": "old ISO time with offset", "tz": "IANA zone" } } and "changeEvidence": "exact short quote from this email confirming the change". Only use replaces when the email explicitly identifies the original and new flights. Ordinary outbound and return legs are separate bookings, not replacements. Do not infer a replacement just because two flights share a route or booking reference.\n- Include the airport IANA timezone in tz for each departure and arrival, independently.\n- confirmationCode is the airline booking reference / PNR, not the agency customer reference, order number or PIN.\n- emailIndex corresponds to the position in the array below.\n\nEmails:\n${JSON.stringify(emailBodies)}` }],
    });

    let text = responseText(extraction)?.trim();
    if (text) {
  ...
  const { reconcileFlights } = require("./flight-bookings");
  // The scanner's own clock, so the reconciler judges 'past' by the same time it does.
  const result = reconcileFlights(ctx.store.facts, allParsedFlights, emailBodies, Date.now());
  for (const [key, flight] of result.patches) _setNoteValue(key, JSON.stringify(flight));
  if (result.patches.length) saveStore();
...
async function scanCalendarForFlights(userId) {
  ...
  for (const ev of (events || [])) {
    ...
    await ensureFlightTimezones(noteData);
...
// Resolve each airport independently, including flights outside the live window.
async function ensureFlightTimezones(flight) {
  for (const end of ["departure", "arrival"]) {
    const point = flight[end];
    if (!point) continue;
    FlightTime.preserveOffset(point);
    if (!point.tz && point.airport) point.tz = await getAirportTz(point.airport);
  }
}
...
async function checkFlightsForUpdates(userId) {
  ...
  for (const [key, value] of Object.entries(ctx.store.facts)) {
    ...
    if (flight.landed || flight.supersededBy) continue;

    if (!FlightTime.shouldTrack(flight, now)) continue;
    ...
    flight.liveCheckedAt = new Date(now).toISOString();
...
function buildFlightBriefing(flight) {
  ...
  const depTime = FlightTime.time(live.departureTime || dep.dateTime, dep);
  const arrTime = FlightTime.time(live.arrivalTime || arr.dateTime, arr);
  ...
  lines.push(`Departs ${depTime}${depTime !== "--:--" ? ` (${dep.airport || "departure airport"} local)` : ""}${terminalGate ? ` from ${terminalGate}` : ""}`);
  lines.push(`Arrives ${arrTime}${arrTime !== "--:--" ? ` (${arr.airport || "arrival airport"} local)` : ""}`);
...
async function sendFlightPin(userId, noteKey, flight, chatLinks, targets) {
  ...
  for (const link of targets) {
    try {
      let msgId;
      const url = await dashboardUrl(link.platform, "schedules");
