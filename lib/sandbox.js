...
async function destroySandbox(userId) {
  ...
  const { error: destroyErr } = await supabase.from("sandboxes")
    .update({ status: "destroyed" })
    .eq("user_id", userId);
  // The container is gone either way. A lost write leaves a row claiming it is
  // running, which the janitor will keep trying to reap forever.
  if (destroyErr) console.error(`[Sandbox] destroyed but still marked running for ${userId}: ${destroyErr.message}`);
...
async function ensureSandbox(userId) {
  ...
  if (!isStatic()) {
    const { error: touchErr } = await supabase.from("sandboxes")
      .update({ last_used_at: new Date().toISOString() })
      .eq("user_id", userId);
    // This timestamp is what the janitor reads as "idle". A lost write makes a
    // sandbox someone is using look abandoned, and it gets reaped mid-session.
    if (touchErr) console.error(`[Sandbox] could not refresh last_used_at for ${userId}: ${touchErr.message}. It may be reaped while in use.`);
  }
...
async function sandboxExec(userId, language, code, timeoutMs = 30000) {
  ...
  const { error: execErr } = await supabase.from("sandboxes")
    .update({
    ...
    .eq("user_id", userId);
  // Same reaping consequence as above, plus the usage numbers the
  // dashboard shows.
  if (execErr) console.error(`[Sandbox] could not record exec for ${userId}: ${execErr.message}`);
