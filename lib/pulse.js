...
async function runPulseForUser(userId) {
  try {
    ...
    const verdict = await require("./pulse-triage").triage({
      settings, items: triageItems, level,
      now: require("./timezone").nowStamp({ location: store.location, profile: store.profile }),
      fallback: (system, message, tokens) => cheapChat(userId, system, message, tokens),
    });
    if (!verdict.pulse || !Array.isArray(verdict.flagged) || verdict.flagged.length === 0) {
      console.log(`Pulse: ${userId} ${verdict.via || "support LLM"} triage says nothing pulse-worthy (${triageItems.length} new items screened)`);
      await markChecked();
      return;
    }
    console.log(`Pulse: ${userId} ${verdict.via || "support LLM"} triage flagged ${verdict.flagged.length} item(s), running composer`);

    // ---- STAGE 4: full composer (configured support LLM) ----
...
- If NO: respond with exactly [NO_PULSE] and nothing else.
- If YES: call send_pulse with the message. Tell them ONLY the new information. Do not repeat what you said before.

STEP 4 - SAVE:
After send_pulse, save a brief note via pin_fact("_pulse_last", <what you told them>) so you don't repeat next time.

HOW THE MESSAGE REACHES THE PHONE:
The ONLY thing ${userName} ever sees is the text you pass to send_pulse. Nothing you write outside that call is delivered: not your reasoning, not a summary of what you did, not "Sent." Call send_pulse at most once, with the whole message. If you have nothing to send, do not call it.
...
- The message is read cold, on a phone, hours after anything else you said. Name the thing, say why it matters now and what to do, in full. Never refer to an earlier message, to "the other two", or to anything the reader would have to scroll back for.
...
- FLIGHTS: If the user has flights tracked (check flight_scan), upcoming departures within 24h are always worth a pulse with gate/terminal/time info.`;
    ...
    // The message is a tool call, not the model's closing words. When the
    // closing text was the message, the model sometimes wrote the nudge
    // into its _pulse_last note and closed with a report on its own work
    // ("Sent. The message went out covering..."), and the report reached
    // the phone as if it were the nudge. A tool makes the boundary hard:
    // what goes to send_pulse is delivered, everything else is discarded.
    const SEND_TOOL = {
      name: "send_pulse",
      description: `Send the nudge to ${userName}'s phone. The text is the entire message they will see, read cold, hours after anything else. Call at most once per check. Do not call it if there is nothing new worth sending.`,
      input_schema: { type: "object", properties: { text: { type: "string", description: "The whole message, 2-4 sentences, no preamble." } }, required: ["text"] },
    };
    let sent = false;
    const deliver = async (text) => {
      for (const t of targets) {
        try { await sendToPlatform(t.platform, t.platform_user_id, text); } catch (e) { console.error(`Pulse send failed via ${t.platform}: ${e.message}`); }
      }
      sent = true;
      // Save the message into the conversation so a reply to it lands on
      // something the model can see. This ran with no user context loaded
      // and never saved, so the text went into an empty in-memory store and
      // "the other two what?" reached a model that had never said it.
      try {
        await acquireUserMutex(userId, async () => {
          ...
          getConversation(userId).push({ role: "assistant", content: text });
          syncAdapterBack();
          await ctx.activeUserStore.save();
          cleanupUserContext();
        });
    ...
    let messages = [{ role: "user", content: `Triage flagged these new items as potentially worth a nudge:\n${verdict.flagged.map(f => "- " + f).join("\n")}\n\nVerify with your tools, check _pulse_last so you don't repeat yourself, and decide whether to message me. If yes, the message goes through send_pulse and nothing else reaches me. [NO_PULSE] is still the right answer if these turn out to be stale or already known.` }];
    let iteration = 0;
    ...
    while (iteration < MAX_PULSE_ITERATIONS) {
      ...
      const response = await llm.messages.create({
        ...
        system: fullPrompt,
        messages,
        tools: [...tools, SEND_TOOL],
      });

      if (response.stop_reason !== "tool_use") {
        // Done. The closing text is never delivered; only send_pulse is.
        let finalText = "";
        ...
        } else if (!finalText || /\bNO[_ ]?PULSE\b/i.test(finalText)) {
          console.log(`Pulse: nothing to report for ${userName} (${toolsUsed.size} tools checked)`);
        } else {
          // Words with no send_pulse call: reasoning, a write-up, a report on
          // its own work. Logged so a lost nudge can be seen, never sent.
          console.log(`Pulse: composer closed without send_pulse for ${userName}, discarded: ${finalText.slice(0, 120).replace(/\s+/g, " ")}`);
        }
        return;
      }
      ...
      for (const block of response.content) {
        ...
        if (block.name === "send_pulse") {
          const text = String((block.input && block.input.text) || "").trim();
          let result;
          if (sent) result = { error: "Already sent this check. One message per check." };
          else if (!text || /\bNO[_ ]?PULSE\b/i.test(text)) result = { error: "Nothing to send. If there is nothing new, just reply [NO_PULSE] without calling send_pulse." };
          else if (/^\s*#{1,6}\s/.test(text)) result = { error: "That reads as a document, not a message. Send 2-4 plain sentences, no headings." };
          else { await deliver(text); result = { sent: true }; }
    ...
    console.log(`Pulse hit max iterations for ${userName}${sent ? " (message already sent)" : ""}`);
    if (sent) await markChecked();
  } catch (e) {
