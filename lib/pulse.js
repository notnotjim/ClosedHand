...
- If NO: respond with exactly [NO_PULSE] and nothing else. Never explain a decision not to message; the user never sees the reasoning, only the message, so any words around the token would reach their phone.
...
- FLIGHTS: If the user has flights tracked (check flight_scan), upcoming departures within 24h are always worth a pulse with gate/terminal/time info.`;
    ...
    while (iteration < MAX_PULSE_ITERATIONS) {
      ...
      if (response.stop_reason !== "tool_use") {
        ...
        // The no-send signal, however the model writes it: [NO_PULSE],
        // NO_PULSE, "# NO_PULSE", "no pulse". One verifier wrote it as a
        // heading over its reasoning and the whole thing reached the phone.
        if (!finalText || /\bNO[_ ]?PULSE\b/i.test(finalText)) {
          console.log(`Pulse: nothing to report for ${userName} (${toolsUsed.size} tools checked)`);
          await markChecked();
          return;
        }
        // A pulse is a chat message of a few sentences. Text that opens with
        // a markdown heading is the model writing a document, its own
        // reasoning, not something for a phone.
        if (/^\s*#{1,6}\s/.test(finalText)) {
          console.log(`Pulse: dropped a write-up for ${userName}: ${finalText.slice(0, 80).replace(/\s+/g, " ")}`);
          await markChecked();
          return;
        }
        ...
        try {
          await acquireUserMutex(userId, async () => {
            ...
            getConversation(userId).push({ role: "assistant", content: finalText.trim() });
            syncAdapterBack();
