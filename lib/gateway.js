...
const { publicRequest, parsePublicURL, serviceURLAllowed } = require("./workspace-network");
...
router.post("/api", verifySandboxToken, async (req, res) => {
  ...
  if (!serviceURLAllowed(service, url)) return res.status(403).json({ error: "This URL does not belong to the selected service." });
  ...
  try {
    ...
    switch (service) {
      ...
      case "whatsapp":
        result = await publicRequest(method.toUpperCase(), url, body || null, {
          ...(extraHeaders || {}),
      ...
      case "slack":
        ...
        break;
      default:
        result = await publicRequest(method.toUpperCase(), url, body || null, extraHeaders || {});
    }
...
router.post("/fetch", verifySandboxToken, async (req, res) => {
  ...
  try {
    const result = await publicRequest(method.toUpperCase(), url, body || null, headers || {});
