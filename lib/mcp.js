...
const { isUserMcpTool, callUserMcpTool, getUserMcpToolDefs, getUserMcpTools } = require("./user-mcp");
...
// Tools the user connected through the dashboard live in user-mcp.js and
// carry the umcp_ prefix. Routing them here means every runner (agents,
// automations, pulse, schedules, confirmations) reaches them through the one
// call it already makes.
async function callMCPTool(toolName, toolInput) {
  if (isUserMcpTool(toolName)) {
    const userId = (toolInput && toolInput._userId) || ctx.activeUserId;
    try {
      return await callUserMcpTool(userId, toolName, toolInput);
    } catch (error) {
...
// Full definitions of every MCP tool this user can call: the install's own
// servers plus the ones they connected. Background runners include these
// outright; the chat turn defers them (see engine.js).
function getMcpToolDefsFor(userId) {
  const defs = ctx.allMcpTools.map((tool) => ({
    name: tool.name,
    description: tool.description || "",
    input_schema: tool.inputSchema || { type: "object", properties: {} },
  }));
  return [...defs, ...getUserMcpToolDefs(userId)];
}

// Open the user's connections before a run builds its tool list.
async function warmUserMcp(userId) {
  if (!userId) return;
