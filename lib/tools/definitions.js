...
const INTERNAL_TOOLS = [
  {
    name: "delegate_email_thread",
    core: true,
    groups: ["settings", "email"],
    description: "Allow or stop bounded replies in an existing assistant-email conversation. Requires owner confirmation showing the exact shared details and recipients. Use its recorded email thread ID. Never grant private inbox, memory or tool access to a guest. Replies can discuss the shared brief; actions beyond it still need the owner. A guest must address the assistant directly to request a reply. Pass only the details the owner explicitly permits sharing.",
    input_schema: { type: "object", properties: { thread_id: { type: "string" }, stop: { type: "boolean" }, purpose: { type: "string" }, shared_brief: { type: "string" }, participants: { type: "array", items: { type: "string" } } }, required: ["thread_id"] },
  },
  {
    name: "recall_sources",
    groups: ["memory", "settings"],
    description: "Inspect which connected sources ClosedHand remembers automatically, their coverage and why a collection stays on demand. Use before explaining missing source context or configuring a source reader. Returns connection IDs, approved API origins and status, never credentials.",
    input_schema: { type: "object", properties: {} },
  },
  {
    name: "configure_source_recall",
    groups: ["memory", "settings"],
    description: "Configure automatic recall for an existing connection when the user asks. Supply its published OpenAPI document or a data-only collection recipe based on its actual tool schema. Never invent endpoints or fields. Durable personal/work records belong in recall; current weather, prices, telemetry, search results and action confirmations stay on demand. The background runner validates the description independently of the LLM. Use recall_sources to find the connection ID and approved API addresses. Descriptions never contain credentials or executable code.",
    input_schema: { type: "object", properties: { kind: { type: "string", enum: ["connection", "mcp"] }, id: { type: "string" }, description: { type: "object", properties: { enabled: { type: "boolean" }, openapi: { type: "object" }, openapiUrl: { type: "string" }, collections: { type: "array", items: { type: "object" } } } } }, required: ["kind", "id", "description"] },
  },
  {
    name: "get_dashboard_overview",
    core: true,
    groups: ["dashboard"],
    description: "Get the current Agents dashboard link and its saved contents: running/recent agents, automations, reminders, flights and hotel/other bookings. Use when asked to open or link the dashboard/Agents page, or show its upcoming schedule. Always fetch the URL here rather than guessing it from chat history. This is read-only and does not start an agent.",
    input_schema: { type: "object", properties: {} },
    activityDescription() { return "Checking your dashboard"; },
  },
  {
    name: "add_schedule",
    groups: ["scheduling"],
    description:
      "Set a reminder for one specific moment, like 'remind me at 3pm on Friday' or 'check on this after my flight lands'. Runs the prompt then and replies in the chat it was set up in. For a relative moment ('in 20 minutes', 'in two hours') pass in_minutes and nothing else about time; for an exact moment pass run_at as ISO 8601 with the user's offset; the tool works out the schedule itself, so never do that arithmetic yourself. cron_expression is only for genuinely recurring patterns, and then run_once should be false. The user never sees the cron line, so your reply must state when it will fire, using the next_run time the tool returns, and pass on any note about the timezone. For anything that recurs, use automation_create instead, so it lands with the user's other agents where they can see it, pause it, and give it skills.",
    input_schema: {
      type: "object",
      properties: {
        name: {
        ...
        in_minutes: {
          type: "number",
          description: "Fire this many minutes from now (1 to 1440). Use this for 'in N minutes/hours'. The tool computes the time; do not also pass cron_expression or run_at.",
        },
        run_at: {
          type: "string",
          description: "Fire at this exact moment, ISO 8601 with the user's UTC offset, e.g. 2026-09-12T15:00:00+09:00. Use this for 'at 3pm on Friday'. Do not also pass cron_expression.",
        },
        cron_expression: {
          type: "string",
          description:
            "Only for a recurring pattern. Cron expression for when to run, in the user's local timezone. Examples: '0 20 * * 0' = every Sunday 8pm, '0 8 * * 1-5' = weekday 8am, '*/30 * * * *' = every 30 minutes. Format: minute hour day-of-month month day-of-week. When the user gives a day but no time ('remind me tomorrow'), pick a sensible hour and tell them what you picked; they said nothing about a time, so they do not know it yet.",
        },
        prompt: {
          type: "string",
          description:
  ...
  },
  {
    name: "list_schedules",
    groups: ["scheduling"],
    description: "List active reminder tasks only. For the dashboard/Agents page link or a full upcoming overview including flights and hotels, use get_dashboard_overview.",
    input_schema: { type: "object", properties: {} },
  },
  {
  ...
  },
  {
    ...
    groups: ["web"],
    description:
      "Fetch and read the text content of a specific URL. Use when the user shares a link or when you need to read a full article/page from a search result. Returns plain text extracted from HTML. This is a plain request with no JavaScript run: a page that says it needs JavaScript, or a filter that only works in the browser, is read with sandbox_browse, which is a real browser.",
    input_schema: {
      type: "object",
      properties: {
        url: {
  ...
  // --- Sentinel: Unified Email/Calendar/Attachment Engine ---
  {
    ...
    core: true,
    groups: ["data"],
    description: "Search the synced data cache: emails and calendar events from ALL connected mail accounts (Gmail including extra accounts, Outlook), WhatsApp messages from every chat when WhatsApp is linked (source 'whatsapp', each with chat_name, sender and date; sender 'me' is the user), plus Slack messages and Notion pages when connected. For 'the latest' or 'the last message' use query '*' with a source: it returns the newest items, no matching. Returns relevant excerpts with source IDs and metadata. Excerpts are not the whole record. Use read_cached_record with its id and source to inspect the full saved original, especially before claiming something is absent. Use for precise lookups: 'emails from john@acme.com', 'Slack messages about launch'. For other services (Shopify, Stripe, etc.) query their APIs live instead. Filter by type and/or source for targeted searches. Emails carry the account they belong to, plus draft_id on drafts, and an attachments list (filenames only, not contents): open the images with fetch_attachment before describing a message that has them. Emails also carry is_draft, and an unsent draft is an outstanding action the user owes someone, so check drafts first when asked what needs a reply or who is chasing.",
    input_schema: {
      type: "object",
      properties: {
        query: { type: "string", description: "Search terms." },
        type: { type: "string", description: "Filter by data type: 'email', 'event', 'message', 'page', 'ticket', 'task', 'item'. Omit for all types." },
        source: { type: "string", description: "Filter by source: 'gmail', 'outlook', 'gcal', 'whatsapp', 'slack', 'notion', 'jira', etc. Omit for all sources." },
  ...
  {
    name: "read_cached_record",
    groups: ["data"],
    description: "Read an original saved source after search_cache. Preserves exact wording, amounts and dates. Large records are paged; follow next_offset to read the rest before claiming something is absent. Nothing is sent or changed.",
    input_schema: { type: "object", properties: {
      id: { type: "string", description: "The record id from search_cache." },
      source: { type: "string", description: "Its exact source from search_cache, such as gmail or outlook." },
      offset: { type: "integer", description: "Character offset. Start at zero; use next_offset for the next page." },
      limit: { type: "integer", description: "Characters per page, up to 16000. Default 8000." }
    }, required: ["id", "source"] },
  },
  {
  ...
  // --- OneDrive Tools (Microsoft 365) ---
  {
    name: "onedrive_search",
    groups: ["onedrive"],
    description: "Search OneDrive for files by name or content, or list files inside a folder. Returns file names, types, links, and IDs. If a result is a folder, you can search again with its ID as folder_id to list its contents. Searches every connected Microsoft account by default; name an account to limit it.",
    input_schema: {
      type: "object",
      properties: {
        query: { type: "string", description: "Search query — matches file names and content. E.g. 'budget 2026', 'gym invoice'. Can be omitted when using folder_id to list folder contents." },
        folder_id: { type: "string", description: "Optional: OneDrive folder ID. When provided, lists files inside this folder instead of searching." },
        max_results: { type: "number", description: "Max files to return (default 10, max 20)" },
        account: { type: "string", description: "Optional: which Microsoft account to use (email or fragment, e.g. 'work'). Omit to search every account." },
      },
    },
    activityDescription(input) {
      if (input.query) return `Searching OneDrive for "${input.query.substring(0, 40)}"`;
      if (input.folder_id) return "Listing folder contents";
      return "Searching your OneDrive";
    },
  },
  {
    name: "onedrive_list_recent",
    groups: ["onedrive"],
    description: "List recently modified files in OneDrive. Covers every connected Microsoft account by default.",
    input_schema: {
      type: "object",
      properties: {
        max_results: { type: "number", description: "Max files to return (default 10, max 20)" },
        account: { type: "string", description: "Optional: which Microsoft account to use (email or fragment, e.g. 'work'). Omit for every account." },
      },
    },
    activityDescription() { return "Listing recent OneDrive files"; },
  },
  {
    name: "onedrive_read",
    groups: ["onedrive"],
    description: "Read the text content of a OneDrive file by its ID. Works with Word (.docx) and Excel (.xlsx) files and plain text, CSV, HTML and JSON files. For PDFs, images, presentations or other files, use onedrive_send_file instead to send the actual file to the user.",
    input_schema: {
      type: "object",
      properties: {
        file_id: { type: "string", description: "OneDrive file ID from onedrive_search or onedrive_list_recent results" },
        account: { type: "string", description: "Optional: which Microsoft account the file is in (the account shown with the search result). Omit for the primary account." },
      },
      required: ["file_id"],
    },
    activityDescription(input) { return `Reading OneDrive file: ${(input.file_id || "").substring(0, 20)}`; },
  },
  {
    name: "onedrive_send_file",
    groups: ["onedrive"],
    description: "Download a file from OneDrive and send it directly to the user in chat. Works with any file type. Use when the user wants to SEE or RECEIVE a file, not just read its text content.",
    input_schema: {
      type: "object",
      properties: {
        file_id: { type: "string", description: "OneDrive file ID from onedrive_search or onedrive_list_recent results" },
    ...
    activityDescription(input) { return `Sending OneDrive file: ${(input.file_id || "").substring(0, 20)}`; },
  },
  // --- Google Maps Tools ---
  {
    name: "maps_search_places",
    groups: ["maps"],
    description: "Search for places, businesses, restaurants, attractions nearby or in a specific area. Returns names, addresses, links to open in Google/Apple Maps, and ratings and opening hours when the source has them. Use when the user asks 'what's nearby', 'find me a restaurant', 'where is the nearest X', etc.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    groups: ["search"],
    description: "Deep hybrid search (meaning AND exact words, fused then reranked) over the synced index: every mail account, calendar, Slack and Notion caches when connected, and past conversation memory. Returns results BEYOND what was already auto-surfaced in your context (no duplicates). Use freely whenever the user wants detail, and for exact identifiers too: invoice numbers and references are matched against full original text. Indexed documents are covered by search_documents instead; to send a stored file use rag_retrieve, or drive_search for live Drive. Use the service filter for targeted searches (e.g. service='email').",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        service: {
          type: "string",
          description: "Service key to connect (e.g. shopify, meta_ads, slack, notion, github, stripe, hubspot, dropbox, salesforce, spotify, gitlab)",
        },
        store_domain: {
          type: "string",
  ...
  },
  {
    name: "list_bookings",
    groups: ["travel"],
    description: "Upcoming bookings picked out of the user's email: trains, hotels, event tickets, restaurant tables, hire cars, ferries. Each carries its reference, times, place and what matters at the door. Flights are separate (list_flights). Meetings are in the calendar, not here.",
    input_schema: {
      type: "object",
      properties: {
        days: { type: "number", description: "How far ahead to look, in days. Default 120." },
      },
    },
  },
  {
    name: "booking_scan",
    groups: ["travel"],
    description: "Read recent confirmation emails now and pick out any bookings not yet on the list (trains, hotels, events, restaurants, cars). Use when the user mentions a booking they expect to see, or asks what is coming up and the list looks thin.",
    input_schema: { type: "object", properties: {} },
  },
  {
    name: "flight_scan",
  ...
  },
  {
    ...
    groups: ["sandbox"],
    description: "Send a generated file from the workspace to the current chat. Use after making a useful chart, visual comparison, report or interactive HTML tool, including when that format directly helps answer the question. PNG/JPEG visuals preview in WhatsApp; HTML opens separately. Only refer to the file or URL after this succeeds.",
    input_schema: {
      type: "object",
      properties: {
        path: { type: "string", description: "File path relative to /workspace" },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        action: {
          type: "string",
          enum: ["batch", "screenshot", "scrape_text", "extract_data", "click", "fill", "press", "wait_for", "eval_js", "fill_card"],
          description: "batch runs a whole sequence on one connection and is preferred: separate calls each cost seconds reconnecting, which is what makes long jobs time out. Single actions for when you need to look before deciding: screenshot, scrape_text, extract_data, click, fill, press, wait_for, eval_js. fill_card types a card from the user's Wallet into the payment form on the page already open (you never see the number; pass card as 'default', a label, or the last four digits); then press the site's own pay button, which ClosedHand puts to the user before it runs. Paying only ever happens this way, on the page: never by calling a site or a payment provider directly.",
        },
        selector: {
          type: "string",
        ...
        card: {
          type: "string",
          description: "For fill_card: which Wallet card, as 'default', its label, or its last four digits.",
        },
        steps: {
          type: "array",
  ...
  // --- The other MCP primitives: resources and prompts on connected servers ---
  {
    name: "mcp_resources",
    description: "List or read resources (documents, files, records) published by one of the user's connected MCP servers. Use action 'list' to see what a server offers, then 'read' with a uri from that list. Servers that have resources are named in the system prompt.",
    input_schema: {
      type: "object",
      properties: {
        server: { type: "string", description: "The connected server's name as listed in the system prompt" },
        action: { type: "string", enum: ["list", "read"], description: "'list' to see resources, 'read' to fetch one" },
        uri: { type: "string", description: "The resource uri to read (from a list result). Required for 'read'." },
      },
  ...
  {
    name: "mcp_prompt",
    description: "Fetch a ready-made prompt (a set of instructions or a template) from one of the user's connected MCP servers, filling in its arguments. Servers that publish prompts are named in the system prompt. Follow what comes back as if the user had written it.",
    input_schema: {
      type: "object",
      properties: {
        server: { type: "string", description: "The connected server's name as listed in the system prompt" },
        name: { type: "string", description: "The prompt's name" },
        arguments: { type: "object", description: "Argument values the prompt asks for, as name: value", additionalProperties: { type: "string" } },
      },
  ...
  {
    name: "matter_get",
    description: "The full live record of an ongoing matter (people and what each said, facts, what is open, decisions). The one-line version is in your instructions under ONGOING MATTERS; use this when you are about to act on one and need every detail, e.g. before writing an update to everyone involved.",
    input_schema: {
      type: "object",
      properties: { matter: { type: "string", description: "The matter's short id from the list, or its title" } },
      required: ["matter"],
    },
  },
  {
    name: "matter_resolve",
    description: "Mark an ongoing matter as concluded so it stops being carried forward, when the user says it is done or it plainly is.",
    input_schema: {
      type: "object",
      properties: { matter: { type: "string", description: "The matter's short id from the list, or its title" } },
      required: ["matter"],
  ...
  {
    name: "use_skill",
    description: "Load the full text of a skill (built-in or one the user installed) when its instructions apply to the task at hand. Skills are listed in the system prompt by name.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    groups: ["rag", "search"],
    description: "Search INSIDE the user's indexed documents (the File Search library) and get the matching passages back, one per document, best passage first. Hybrid: matches meaning, exact words, and filenames, ranked by a reranker. Use for content questions about stored files ('what does the inspection report say about the helmets?') and quote from the returned passages. A weak_match flag means nothing really matched. This reads documents; rag_retrieve sends the file itself, and drive_search covers live Google Drive for files never indexed.",
    input_schema: {
      type: "object",
      properties: {
        query: { type: "string", description: "What to find, in natural language or exact terms" },
        max_results: { type: "number", description: "Max documents to return (default 5, max 10)" },
      },
      required: ["query"],
    },
    activityDescription(input) { return `Searching documents for "${(input.query || "").substring(0, 40)}"`; },
  },
  {
    name: "rag_retrieve",
    groups: ["rag"],
    description: "Retrieve and send an original file the user indexed into File Search on their Cloud Computer or Local Computer. Finds it with the same hybrid search File Search uses (filename, exact words, meaning) or by exact path, then sends the actual file. To READ what a stored document says rather than send it, use search_documents. For Google Drive files, prefer drive_search then drive_read or drive_send_file: those hit Drive live and cover every account, whereas this only reaches what File Search has already indexed. Use this tool for the Computer sources that drive_search cannot see.",
    input_schema: {
      type: "object",
      properties: {
...
// A tool that needs a key this install does not have is not offered at all.
// Offering it and letting it fail costs the model a turn and the person a wait.
function usable(tool) {
  return !tool.needs || !!process.env[tool.needs];
}

module.exports = { INTERNAL_TOOLS, usable };
