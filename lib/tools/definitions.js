...
const INTERNAL_TOOLS = [
  ...
  },
  {
    ...
    groups: ["notes"],
    description:
      "Pin a fact about the user's life to persistent memory: who they are, who matters to them, what they do, durable preferences and dates. Never investigation findings, research results or task output, those belong in your reply and the agent report, and never operational state: a shipment in transit, a customer's delivery address for a parcel, an open invoice are the work of the week, not facts about the user's life, and conversation memory already covers them. The test: will this still be true and worth knowing in six months? Write the value as one or two self-contained sentences a stranger could read cold: full names for people and companies, no conversational fragments, no 'he' or 'that invoice', no trailing clauses lifted from chat, because it will be read months from now with none of today's context. Before saving, ALWAYS check existing facts (get_facts) for conflicts. Then decide: ADD if no existing fact covers this topic. MERGE if an existing fact covers part of this topic (combine old+new into one updated fact, delete the old key). INVALIDATE if an existing fact is directly contradicted (e.g. user moved cities, changed jobs) and delete the old one. SKIP if the information is already saved and unchanged. Never leave contradictory or duplicate facts.",
    input_schema: {
      type: "object",
      properties: {
