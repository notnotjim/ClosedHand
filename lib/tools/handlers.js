...
// OneDrive twins of the Drive helpers. A query with no account named runs
// against every connected Microsoft account and tags each file with the
// account it came from, the way driveQueryAllAccounts does for Google.
const ONEDRIVE = "https://graph.microsoft.com/v1.0/me/drive";
const ONEDRIVE_FIELDS = "$select=id,name,size,lastModifiedDateTime,webUrl,file,folder";
async function oneDriveQuery(accountInput, pathAndQuery) {
  const { listMicrosoftAccounts } = require("../services/microsoft");
  const accounts = accountInput ? [{ serviceKey: microsoftReqFor(accountInput).serviceKey }] : listMicrosoftAccounts(ctx.activeUserStore);
  const settled = await Promise.allSettled(accounts.map(async (a) => {
    const data = await microsoftApiRequest("GET", ONEDRIVE + pathAndQuery, null, null, a.serviceKey);
    return (data.value || []).map((f) => ({ ...f, _account: a.email || undefined }));
  }));
  const failed = settled.find((r) => r.status === "rejected");
  if (failed && settled.every((r) => r.status === "rejected")) throw failed.reason;
  return settled.flatMap((r) => (r.status === "fulfilled" ? r.value : []));
}
// Recently changed files. Microsoft's own "recent" list can come back empty
// (and is being retired), so an empty answer falls back to walking what the
// drive holds, newest first.
async function oneDriveRecent(serviceKey, max) {
  const recent = await microsoftApiRequest("GET", `${ONEDRIVE}/recent?$top=${max}`, null, null, serviceKey).catch(() => ({ value: [] }));
  let files = (recent.value || []).filter((f) => f.file);
  if (!files.length) {
    let next = `${ONEDRIVE}/root/delta?$top=200&${ONEDRIVE_FIELDS},deleted`;
    for (let page = 0; next && page < 5; page++) {
      const data = await microsoftApiRequest("GET", next, null, null, serviceKey);
      files.push(...(data.value || []).filter((f) => f.file && !f.deleted));
      next = data["@odata.nextLink"];
    }
  }
  return files.sort((a, b) => String(b.lastModifiedDateTime || "").localeCompare(String(a.lastModifiedDateTime || ""))).slice(0, max);
}
const ONEDRIVE_MAX_BYTES = 45 * 1024 * 1024;
async function oneDriveFile(accountInput, fileId) {
  const call = microsoftReqFor(accountInput);
  const meta = await call("GET", `${ONEDRIVE}/items/${encodeURIComponent(fileId)}?${ONEDRIVE_FIELDS}`);
  const download = async () => {
    if ((meta.size || 0) > ONEDRIVE_MAX_BYTES) throw new Error(`"${meta.name}" is ${(meta.size / 1048576).toFixed(0)}MB, too large to fetch into chat.`);
    const token = await require("../services/microsoft").getMicrosoftToken(null, call.serviceKey);
    const res = await fetch(`${ONEDRIVE}/items/${encodeURIComponent(fileId)}/content`, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(60000) });
    if (!res.ok) throw new Error(`OneDrive download failed (${res.status})`);
    return Buffer.from(await res.arrayBuffer());
  };
  return { meta, mime: meta.file?.mimeType || "", download };
}
const oneDriveEntry = (f) => ({
  id: f.id,
  name: f.name,
  type: f.folder ? "folder" : f.file?.mimeType || null,
  isFolder: !!f.folder,
  modified: f.lastModifiedDateTime,
  size: f.size && !f.folder ? `${(f.size / 1024).toFixed(0)}KB` : null,
  link: f.webUrl,
  account: f._account,
});
...
async function handleInternalTool(toolName, toolInput) {
  // Never a password reset or change, on either machine, from any runner.
  if (/^(sandbox_browse|bridge_(browser|input|ax)_)/.test(toolName)) {
    const why = require("../spend-guard").touchesPasswordForm(toolName, { ...(toolInput || {}), _userId: (toolInput && toolInput._userId) || ctx.activeUserId });
    if (why) return { error: why };
  }
  switch (toolName) {
    case "delegate_email_thread": return require("../assistant-email").delegate(toolInput);
    case "get_dashboard_overview": {
      const { supabase } = require("../../user-store");
      const { dashboardUrl } = require("../dashboard-links");
      return require("../dashboard-overview").getDashboardOverview({
        db: supabase, userId: toolInput._userId || ctx.activeUserId,
        platform: toolInput._platform || ctx.activePlatform, getUrl: dashboardUrl,
      });
    }
    case "add_schedule": {
      const scheduleTzEarly = require("../timezone").getUserTimezone(ctx.store);
      const tzKnown = !!(ctx.store?.location?.timezone || ctx.store?.profile?.settings?.location?.timezone);
      // A relative or exact moment is turned into the cron line here, in the
      // user's zone, so the model never has to do clock arithmetic (which is
      // how "one minute from now" once landed on a time already gone).
      if (toolInput.in_minutes !== undefined || toolInput.run_at) {
        let when;
        if (toolInput.in_minutes !== undefined) {
          const mins = Number(toolInput.in_minutes);
          if (!Number.isFinite(mins) || mins < 1 || mins > 1440) return { error: "in_minutes must be between 1 and 1440." };
          when = new Date(Date.now() + Math.round(mins) * 60000);
        } else {
          when = new Date(String(toolInput.run_at));
          if (isNaN(when.getTime())) return { error: "run_at must be ISO 8601 with a UTC offset, e.g. 2026-09-12T15:00:00+09:00." };
          if (when.getTime() < Date.now() - 60000) return { error: `run_at is in the past (${when.toISOString()}). Now is ${new Date().toISOString()}.` };
        }
        const parts = new Intl.DateTimeFormat("en-GB", { timeZone: scheduleTzEarly, minute: "numeric", hour: "numeric", day: "numeric", month: "numeric", hour12: false }).formatToParts(when);
        const get = (t) => Number(parts.find((p) => p.type === t).value) % (t === "hour" ? 24 : 1000);
        toolInput.cron_expression = `${get("minute")} ${get("hour")} ${get("day")} ${get("month")} *`;
        toolInput.run_once = true;
        toolInput._computedFrom = when;
      }
      if (!toolInput.cron_expression) return { error: "Say when: pass in_minutes for a relative moment, run_at for an exact one, or cron_expression for a recurring pattern." };
      ...
      if (ctx.activeUserStore) {
        await ctx.activeUserStore.saveSchedule(toolInput.name, toolInput.cron_expression, toolInput.prompt, ctx.activeChatId, toolInput.run_once === true, scheduleTz, toolInput._platform || ctx.activePlatform || null);
      }
      // Update local store
      ...
      return {
        ...
        ...(nextRun ? { next_run: `${nextRun} (${scheduleTz}${tzKnown ? "" : ", assumed: the user's timezone is not set"})` } : {}),
        note: "Tell the user when this fires (the next_run time, in words). If they never said a time themselves, this is their only chance to hear the one you picked. They can review schedules in their dashboard."
          + (tzKnown ? "" : ` The user's timezone is not set, so this assumes ${scheduleTzEarly}; if the exact time matters, ask them to confirm their timezone or set their location.`),
      };
    }
    case "list_schedules": {
    ...
    case "pin_fact": {
      ...
      // An edit that does not restate the category keeps the one it had, so
      // re-saving a note cannot quietly drop it out of its group.
      const category = toolInput.category || (existing && existing.category) || "topic";
      const subject = toolInput.subject || (existing && existing.subject) || null;
      // Where a fact came from stays with it: pinned from chat, guessed by
      // the setup scan, or typed in on the dashboard.
      ...
      saveStore();
      // Mirror into data_vectors so Context Brain shows it and explicit search
      // can reach it. Plumbing keys are skipped by the mirror itself.
      //
      // Non-blocking, because the fact is saved either way, but never silent.
      // This swallowed its error for a week and production ended up with three
      // notes and no mirrors at all, which read as "the mirror does not work"
      // when nothing had said what failed or whether it had even been reached.
      let mirrored = null;
      try {
        mirrored = await _factVectors().mirrorFact(ctx.activeUserId, toolInput.key, toolInput.value);
      } catch (e) {
        console.error(`[Facts] "${toolInput.key}" saved but not mirrored to Context Brain: ${e.message}`);
      }
      ...
      // recall is reported so a failed mirror is visible in the transcript as
      // well as the log. Not something to tell the user about: the fact is
      // pinned, and the mirror is plumbing behind Context Brain.
      return {
        success: true,
        message: `Fact '${toolInput.key}' pinned.`,
        total_facts: factCount,
        ...(mirrored === false ? {} : { recall: mirrored ? "indexed" : "not indexed" }),
      };
    }
    case "get_facts": {
      _migrateFactsMetadata();
      ...
      saveStore();
      // Grouped by category and subject alongside the flat map. This is what
      // makes "is there already a note about this?" a lookup instead of a
      // judgement made by reading every value: find the subject, reuse its key.
      const byCategory = {};
      for (const key of factKeys) {
        const fact = ctx.store.facts[key];
        if (!fact || typeof fact !== "object" || fact.value === undefined) continue;
        const cat = fact.category || "topic";
        (byCategory[cat] = byCategory[cat] || []).push({ key, subject: fact.subject || null });
      }
      return { facts: plainFacts, by_category: byCategory, count: factKeys.length };
    }
    case "delete_fact": {
      _migrateFactsMetadata();
...
DUPLICATE: [number or "none"]` }],
        ...
        try {
          ...
          const distilled = responseText(response)?.trim();
    ...
    case "web_search": {
      if (!BRAVE_API_KEY) {
        // No key: DuckDuckGo's HTML results, same shape. See web-search-fallback.js.
        try {
          const { duckDuckGoSearch } = require("../web-search-fallback");
          const results = await duckDuckGoSearch(toolInput.query, Math.min(toolInput.count || 5, 10));
    ...
    case "gmail_create_draft": {
      ...
      // One thread, one unsent draft, unless a second was actually asked for.
      // Two agents working the same thread each created their own reply draft
      // and neither could see the other's, so the user was left choosing
      // between two drafts on one conversation with no way to tell which was
      // current. Refusing with the existing draft's id turns the second
      // attempt into an edit, which is what was meant.
      if (isReply && toolInput.thread_id && !toolInput.allow_duplicate) {
        try {
          const { supabase } = require("../../user-store");
          const { data: drafts } = await supabase
            ...
            .eq("user_id", ctx.activeUserId)
            .eq("type", "email")
            .eq("data->>threadId", toolInput.thread_id)
            .eq("data->>is_draft", "true")
            .limit(1);
          const existing = drafts?.[0]?.data;
          if (existing?.draft_id) {
            return {
              error: `This thread already has an unsent draft (draft_id ${existing.draft_id}${existing.account ? `, account ${existing.account}` : ""}). Edit it with gmail_draft_update, which keeps its attachments, rather than adding a second one. If the user genuinely wants a second draft alongside it, call this again with allow_duplicate true.`,
            };
          }
        } catch (e) {
          // A cache miss must not block a draft the user asked for.
          console.log(`[gmail_create_draft] Could not check for an existing draft: ${e.message}`);
        }
      }
    ...
    case "onedrive_search": {
      ...
      try {
        ...
        let raw;
        if (toolInput.folder_id) {
          // A folder id belongs to one account, so a listing stays single account.
          raw = await oneDriveQuery(toolInput.account || null, `/items/${encodeURIComponent(toolInput.folder_id)}/children?$top=${maxResults}&${ONEDRIVE_FIELDS}`);
          if (!toolInput.account) raw = raw.slice(0, maxResults);
        } else if (toolInput.query) {
          const q = encodeURIComponent(String(toolInput.query).replace(/'/g, "''"));
          raw = await oneDriveQuery(toolInput.account || null, `/root/search(q='${q}')?$top=${maxResults}&${ONEDRIVE_FIELDS}`);
        } else {
          return { error: "Provide either a query or folder_id" };
        }
        const files = raw.slice(0, toolInput.account ? maxResults : maxResults * 2).map(oneDriveEntry);
        return { files, count: files.length, query: toolInput.query || `folder:${toolInput.folder_id}` };
      } catch (e) {
        return { error: `OneDrive search failed: ${e.message}` };
      }
    }
    case "onedrive_list_recent": {
      if (!isMicrosoftConnected()) return { error: "Microsoft not connected — connect it from your dashboard" };
      try {
        const maxResults = Math.min(toolInput.max_results || 10, 20);
        const { listMicrosoftAccounts } = require("../services/microsoft");
        const accounts = toolInput.account ? [{ serviceKey: microsoftReqFor(toolInput.account).serviceKey }] : listMicrosoftAccounts(ctx.activeUserStore);
        const settled = await Promise.allSettled(accounts.map(async (a) =>
          (await oneDriveRecent(a.serviceKey, maxResults)).map((f) => ({ ...f, _account: a.email || undefined }))));
        const raw = settled.flatMap((r) => (r.status === "fulfilled" ? r.value : []));
        if (!raw.length && settled.some((r) => r.status === "rejected")) throw settled.find((r) => r.status === "rejected").reason;
        raw.sort((a, b) => String(b.lastModifiedDateTime || "").localeCompare(String(a.lastModifiedDateTime || "")));
        const files = raw.slice(0, maxResults).map(oneDriveEntry);
        return { files, count: files.length };
      } catch (e) {
        return { error: `OneDrive list failed: ${e.message}` };
      }
    }
    case "onedrive_read": {
      if (!isMicrosoftConnected()) return { error: "Microsoft not connected — connect it from your dashboard" };
      try {
        const { meta, mime, download } = await oneDriveFile(toolInput.account, toolInput.file_id);
        if (meta.folder) return { error: `"${meta.name}" is a folder, not a file. Use onedrive_search with folder_id to list its contents.` };
        const size = meta.size ? (meta.size / 1024).toFixed(0) + "KB" : "unknown";
        let content;
        if (/^text\//.test(mime) || mime === "application/json") {
          content = (await download()).toString("utf8");
        } else if (mime === "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
          try { content = (await mammoth.extractRawText({ buffer: await download() })).value; }
          catch (e) { content = "[Could not extract text from docx]"; }
        } else if (mime === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
          try {
            const workbook = XLSX.read(await download(), { type: "buffer" });
            content = workbook.SheetNames.map((name) => `--- Sheet: ${name} ---\n${XLSX.utils.sheet_to_csv(workbook.Sheets[name])}`).join("\n\n");
          } catch (e) { content = "[Could not extract text from xlsx]"; }
        } else {
          content = `[${mime === "application/pdf" ? "PDF" : "Binary"} file: ${meta.name}${mime ? " (" + mime + ")" : ""} — use onedrive_send_file to send it to the user. Size: ${size}]`;
        }
        if (content.length > 8000) content = content.substring(0, 8000) + "\n\n[Truncated — file was too long]";
        return { name: meta.name, type: mime, content };
      } catch (e) {
        return { error: `OneDrive read failed: ${e.message}` };
      }
    }
    case "onedrive_send_file": {
      if (!isMicrosoftConnected()) return { error: "Microsoft not connected — connect it from your dashboard" };
      const chatId = toolInput._chatId;
      if (!chatId) return { error: "No chat ID available to send file." };
      try {
        ...
        await sendDocument(chatId, buffer, meta.name, MIME_TYPES[ext] || mime || "application/octet-stream", "drive");
        return { success: true, sent: meta.name, type: mime, size: `${(buffer.length / 1024).toFixed(0)}KB` };
      } catch (e) {
        return { error: `OneDrive send failed: ${e.message}` };
      }
    }
    // --- Air Quality Handler ---
    case "air_quality": {
    ...
    case "maps_search_places": {
      if (!GOOGLE_MAPS_API_KEY) {
        // No key: OpenStreetMap, same shape. See maps-fallback.js.
        try {
          const saved = ctx.store.location ? [ctx.store.location.latitude, ctx.store.location.longitude] : null;
          const given = String(toolInput.location || "").match(/^(-?\d+\.?\d*)\s*,\s*(-?\d+\.?\d*)$/);
          const centre = given ? [Number(given[1]), Number(given[2])] : saved;
          return await require("../maps-fallback").searchPlaces(toolInput.query, centre, toolInput.radius || 5000);
        } catch (e) {
    ...
    case "maps_geocode": {
      if (!GOOGLE_MAPS_API_KEY) {
        try { return await require("../maps-fallback").geocode(toolInput.address); }
        catch (e) { return { error: `Geocode failed: ${e.message}` }; }
      }
      try {
              ...
              for (const att of event.attachments) {
                try {
                  ...
                  if (fileInfo.parents && fileInfo.parents.includes(folderId)) {
                    // Same account the event and the folder lookup used. The
                    // lookups above already went through gApi; only the delete
                    // still defaulted to the primary Drive, so on a non-primary
                    // calendar it asked the wrong account to delete an id it
                    // does not hold and left the real file behind.
                    await deleteFromDrive(att.fileId, null, gApi.serviceKey);
                    driveFilesDeleted++;
                  }
                } catch (e) {
    ...
    case "update_settings": {
      ...
      if (toolInput.llm_provider) changed.push("To change models, open Settings and check the complete model setup before applying it.");
    ...
    case "recall_sources": {
      return { sources: await require("../services/recall-settings").list(require("../db").supabase, toolInput._userId || ctx.activeUserId) };
    }
    case "configure_source_recall": {
      return require("../services/recall-settings").configure(require("../db").supabase, toolInput._userId || ctx.activeUserId, toolInput.kind, toolInput.id, toolInput.description);
    }
    case "api_request": {
    ...
    case "list_bookings": {
      const { listUpcoming } = require("../bookings");
      const rows = await listUpcoming(toolInput._userId || ctx.activeUserId, { days: Number(toolInput.days) || 120 });
      return {
        count: rows.length,
        bookings: rows.map((b) => ({
          id: b.id, kind: b.kind, title: b.title, provider: b.provider, reference: b.reference,
          starts_at: b.starts_at, ends_at: b.ends_at, timezone: b.timezone, location: b.location, status: b.status, details: b.details || {},
        })),
        note: rows.length ? "Times are stored in UTC; give them to the user in the booking's own timezone." : "Nothing on the list. booking_scan reads recent confirmation emails for any that were missed.",
      };
    }

    case "booking_scan": {
      const { scanBookings } = require("../bookings");
      const r = await scanBookings(toolInput._userId || ctx.activeUserId);
      return { scanned_emails: r.scanned, bookings_seen: r.found, newly_added: r.new, added: r.added || [], ...(r.error ? { error: r.error } : {}) };
    }

    case "list_flights": {
      const { getFlightStatus, buildFlightBriefing, ensureFlightTimezones } = require("../flights");
      const now = Date.now();
      const flights = [];
      ...
      for (const [key, value] of Object.entries(ctx.store.facts)) {
        ...
        try {
          ...
          if (flight.supersededBy) continue;
          await ensureFlightTimezones(flight);
          ...
          flights.push({
            ...
            departure: require("../dashboard-overview").localPoint(flight.departure, flight.liveStatus?.departureTime, now),
            arrival: require("../dashboard-overview").localPoint(flight.arrival, flight.liveStatus?.arrivalTime, now),
      ...
      const schedules_url = await require("../dashboard-links").dashboardUrl(toolInput._platform || ctx.activePlatform, "schedules").catch(() => null);
      return { flights, count: flights.length, schedules_url,
        note: "Use the airport-local date, time and relative_day supplied here. Share schedules_url when reporting itinerary changes. A missing arrival time is unconfirmed." };
    }
    ...
    case "flight_scan": {
      ...
      const schedules_url = await require("../dashboard-links").dashboardUrl(toolInput._platform || ctx.activePlatform, "schedules").catch(() => null);
      const { localPoint } = require("../dashboard-overview");
      return {
        message: newFlights.length ? `Found ${newFlights.length} new or changed flight(s). The saved itinerary is updated.`
          : "No new or changed flight bookings in this scan. Existing tracked flights may still be present; use list_flights to read them.",
        schedules_url,
        note: "Share schedules_url so the user can review the itinerary. Times and relative days are airport-local. Email updates do not require live tracking; gate and live status checks require a configured flight data service.",
        flights: newFlights.map(f => ({
          flightNumber: f.flightNumber,
          airline: f.airline,
          departure: localPoint(f.departure),
          arrival: localPoint(f.arrival),
          confirmationCode: f.confirmationCode,
          scheduleChanges: f.scheduleChanges,
          replacesFlightNumber: f.replacesFlightNumber,
        })),
      };
    }
    ...
    case "sandbox_exec": {
      ...
      try {
        ...
        const seconds = Math.min(120, Math.max(1, Number(toolInput.timeout_seconds) || 30));
        const result = await sandboxExec(ctx.activeUserId, toolInput.language, toolInput.code, seconds * 1000);
    ...
    case "sandbox_file_download": {
      ...
      try {
        ...
        if (isCanvas) {
          ...
          const { data: canvasRow, error: canvasError } = await supabase.from("canvases").insert({
          ...
          if (canvasError && ctx.activePlatform === "web") return { error: "Could not save the generated file for web chat." };
          if (canvasError) console.warn("[Canvas] Could not save a browser preview; sending the file in chat.");
          if (canvasRow) {
            canvasUrl = await require("../dashboard-links").canvasUrl(ctx.activePlatform, canvasRow.id);
        ...
        if (ctx.activePlatform !== "web") {
          ...
          if (isCanvas && (canvasUrl || ctx.activePlatform === "whatsapp" || ctx.activePlatform === "whatsapp_linked")) {
        ...
        if (isCanvas && ctx.activePlatform === "web") result.message += ". Available in the web canvas panel.";
        if (canvasUrl) result.canvas_url = canvasUrl;
        return result;
      } catch (e) {
    ...
    case "sandbox_upload": {
      ...
      try {
        ...
        // Disk first, storage fallback. Reading att.filePath directly failed
        // with "path must be of type string, received undefined" on anything
        // rehydrated from the database, and would still miss a file this
        // container has never held: attachments outlive the disk they were
        // written to. loadAttachmentRaw is the reader that already knows both.
        const { loadAttachmentRaw } = require("../attachments");
        const raw = await loadAttachmentRaw(toolInput.attachment_id);
        if (!raw) return { error: `Attachment ${toolInput.attachment_id} is no longer available.` };
        const base64 = raw.buffer.toString("base64");
        const dest = toolInput.destination || raw.fileName;
...
`;
            break;
          case "fill_card": {
            // A card from the user's Wallet, typed into the payment form on the
            // current page. The number is decrypted here and goes straight to
            // the browser; the model only learns which fields were filled.
            const wallet = require("../wallet");
            const card = await wallet.findCard(userId, toolInput.card);
            if (!card) return { error: toolInput.card && toolInput.card !== "default" ? `There is no card matching "${toolInput.card}" in the user's Wallet.` : "The user has no card in their Wallet. Ask them to add one in Settings, or to pay this one themselves." };
            const secret = await wallet.loadSecret(userId, card.id);
            if (!secret) return { error: "The card could not be read. ClosedHand's encryption key may have changed; the user can re-add the card in Settings." };
            if (browseUrl) {
              // Navigating and filling in one go is never right: the payment
              // form should already be in front of us.
              return { error: "fill_card works on the page already open. Navigate first, get to the payment form, then call fill_card without a url." };
            }
            code = pyImport("_page") + wallet.fillScript(secret);
            wallet.noteCardFill(userId, card);
            break;
          }
          default:
        ...
        console.log(`[browse] ${action}${browseUrl ? " " + browseUrl : ""}${action === "batch" && Array.isArray(toolInput.steps) ? " " + toolInput.steps.map((st) => st && st.action).join(">") : ""}`);
        ...
        if (result.stdout) {
          try {
            const parsed = JSON.parse(result.stdout);
            // Remember where the browser is, so a click that pays on a checkout
            // page can be recognised without another round trip.
            const last = Array.isArray(parsed.results) && parsed.results.length ? parsed.results[parsed.results.length - 1] : parsed;
            if (last && last.url) require("../spend-guard").notePage(userId, last.url, last.title);
            return parsed;
          } catch { /* return raw */ }
        }
        return result;
    ...
    case "agent_start": {
      ...
      const { dashboardUrl, agentLinkNotice } = require("../dashboard-links");
      const dashboard_url = await dashboardUrl(platform);
      return {
        success: true,
        task_id: result.taskId,
        dashboard_url,
        dashboard_note: await agentLinkNotice(platform),
        model: result.model,
    ...
    case "agent_note": {
      ...
      const { supabase } = require("../../user-store");
      const note = { id: require("crypto").randomUUID(), note: String(toolInput.note), at: new Date().toISOString() };
      const { data: updated, error: noteErr } = await supabase.rpc("append_agent_note", {
        p_task_id: target.id, p_user_id: noteUserId, p_note: note });
      if (noteErr) return { error: "The correction could not be saved: " + noteErr.message };
      if (!updated?.length) return { error: "That task has stopped. The correction was not applied." };
      return {
        success: true,
        task_id: target.id,
        message: `Passed to "${(target.title || target.goal || "the running agent").substring(0, 60)}". The correction is saved and will be read at its next step; do not claim it has already acted on it.`,
      };
    }
    ...
    case "mcp_resources": {
      const { listUserMcpResources, readUserMcpResource } = require("../user-mcp");
      const uid = toolInput._userId;
      try {
        if (toolInput.action === "read") {
          if (!toolInput.uri) return { error: "uri is required to read a resource" };
          return { server: toolInput.server, uri: toolInput.uri, content: await readUserMcpResource(uid, toolInput.server, toolInput.uri) };
        }
        return await listUserMcpResources(uid, toolInput.server);
      } catch (e) {
        return { error: e.message };
      }
    }
    ...
    case "mcp_prompt": {
      const { getUserMcpPrompt } = require("../user-mcp");
      try {
        const text = await getUserMcpPrompt(toolInput._userId, toolInput.server, toolInput.name, toolInput.arguments || {});
        return { server: toolInput.server, prompt: toolInput.name, text };
      } catch (e) {
        return { error: e.message };
      }
    }
    ...
    case "matter_get": {
      const m = await require("../matters").getMatter(toolInput._userId || ctx.activeUserId, toolInput.matter);
      if (!m) return { error: `No matter matches '${toolInput.matter}'. The open ones are listed in your instructions.` };
      return { id: String(m.id).slice(0, 8), title: m.title, status: m.status, summary: m.summary, expected_end: m.expected_end, last_touched: m.last_touched, state: m.state };
    }

    case "matter_resolve": {
      const mm = require("../matters");
      const m = await mm.getMatter(toolInput._userId || ctx.activeUserId, toolInput.matter);
      if (!m) return { error: `No matter matches '${toolInput.matter}'.` };
      await mm.resolveMatter(toolInput._userId || ctx.activeUserId, m.id);
      return { resolved: m.title };
    }

    case "use_skill": {
      const { getBuiltInSkill } = require("../skills");
      const { getUserSkill } = require("../user-skills");
      const built = getBuiltInSkill(toolInput.name);
      if (built) return { name: built.name, text: built.body };
      const mine = getUserSkill(toolInput._userId, toolInput.name);
      if (mine) return { name: mine.name, text: mine.body };
      return { error: "There is no skill called '" + toolInput.name + "'. Use a name from the list in your instructions." };
    }
    ...
    case "search_cache": {
      try {
        // Anything that is not a mail search goes to the universal path. A
        // source without a type used to fall through to mail and answer a
        // WhatsApp question with Gmail.
        if (!require("../search-route").wantsMail(toolInput)) {
    ...
    case "read_cached_record": {
      const { supabase } = require("../../user-store");
      return require("../task-retrieval").readCachedRecord(supabase, toolInput._userId, toolInput);
    }
    ...
    case "search_documents": {
      const userId = ctx.activeUserId || toolInput._userId;
      if (!userId) return { error: "No user context" };
      const found = await _docSearch().searchDocuments(userId, toolInput.query);
      if (found.error) return { error: found.error };
      const max = Math.min(toolInput.max_results || 5, 10);
      const results = (found.results || []).slice(0, max).map(r => ({
        document: r.document_name,
        origin: r.origin,
        passage: (r.content || "").substring(0, 900),
        matching_sections: r.chunk_matches,
        relevance: r.relevance,
        ...(r.low_confidence ? { weak_match: true } : {}),
      }));
      if (results.length === 0) return { results: [], message: "Nothing in the indexed library matches that. drive_search (Google Drive) and onedrive_search (OneDrive) reach the live files if it was never indexed." };
      return {
        results,
        ...(found.no_strong_matches ? { note: "Nothing closely matches. The nearest content is returned, flagged weak_match; treat it as a lead, not an answer." } : {}),
      };
    }
    ...
    case "rag_retrieve": {
      ...
      try {
        ...
        if (toolInput.file_path && toolInput.origin) {
          ...
          // The same retrieval File Search runs: both arms, rerank on name
          // plus body, one row per document, exact-name tier. The tier is
          // what makes "send me P1011603" pick the file CALLED that rather
          // than the chunk most about it.
          const found = await _docSearch().searchDocuments(userId, toolInput.query);
          if (found.error) return { error: "Document search failed: " + found.error };
          const top = (found.results || [])[0];
          if (!top) return { error: "No matching document found in library" };
          if (!top.file_path) return { error: "Document source path unavailable" };
          filePath = top.file_path; origin = top.origin; fileName = top.document_name;
        } else {
...
// Same lazy shape as _factVectors, same reason. The deps mirror what the
// webapp injects from its own side of the vendored copy: embed is quick
// because a user is waiting on this path.
let _ds = null;
function _docSearch() {
  if (!_ds) {
    ...
    _ds = docSearch({
      ...
      embed: (text) => require("../services/usi").embedText(text, { quick: true }),
      rerank: (q, docs, topK) => require("../services/reranker").rerank(q, docs, topK),
    });
  }
  return _ds;
}
