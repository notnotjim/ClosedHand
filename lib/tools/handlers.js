...
async function handleInternalTool(toolName, toolInput) {
  switch (toolName) {
    ...
    case "pin_fact": {
      ...
      saveStore();
      // Embed fact into data_vectors for semantic search
      // Plumbing keys are state, not memories; embedding them puts pulse
      // logs into the user's Context Brain where they read as things
      // ClosedHand "knows about" them.
      if (!isInternalFactKey(toolInput.key)) try {
        const { embedDocument } = require("../services/usi");
        const sb = require("../../user-store").supabase;
        const factText = `${toolInput.key}: ${toolInput.value}`;
        const embedding = await embedDocument(factText);
        if (embedding) {
          await sb.from("data_vectors").upsert({
            user_id: ctx.activeUserId,
            service: "memory",
            item_type: "fact",
            external_id: `fact_${toolInput.key}`,
            content: factText,
            embedding: JSON.stringify(embedding),
            enrichment_level: "full",
            source_metadata: { key: toolInput.key },
    ...
    case "delete_fact": {
      ...
      if (ctx.store.facts[toolInput.key]) {
        ...
        // Also delete the fact vector
        try {
          const sb = require("../../user-store").supabase;
          sb.from("data_vectors").delete()
            .eq("user_id", ctx.activeUserId)
            .eq("service", "memory")
            .eq("external_id", `fact_${toolInput.key}`)
            .then(() => {}).catch(() => {});
        } catch (e) { /* non-blocking */ }
...
DUPLICATE: [number or "none"]` }],
    ...
    case "semantic_search": {
      ...
      if (results.results.length > 7 + topK) {
        const reranked = await rerank(toolInput.query, results.results, 7 + topK);
        results.results = reranked.slice(7); // Skip the top 7 that passive recall already showed
      } else if (results.results.length > 7) {
        results.results = results.results.slice(7);
      }
      return results;
    }
...
// JSON into every single message. Same filter the dashboard already applies.
