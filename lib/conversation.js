 ...
 * Summarises oldest messages, embeds the summary into data_vectors, and removes them.
 * No summary blob accumulates. Each chunk is summarised once, vectored once, gone.
 */
async function vectoriseOldMessages(userId, systemPrompt, tools, provider) {
  ...
  try {
    ...
    // Fallback: just trim if summarisation fails
    if (conversation.length > 30) {
      ctx.store.conversations[userId] = conversation.slice(-6);
      saveStore();
    }
  }
}
