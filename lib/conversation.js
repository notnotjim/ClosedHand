...
async function vectoriseOldMessages(userId, systemPrompt, tools, provider) {
  ...
  try {
    ...
    const response = await internalClient.messages.create({
      ...
      messages: [{
        role: "user",
        content: `Summarise this conversation history in 2-3 short paragraphs. Write it as a self-contained overview a stranger could follow months later: full names for people and companies, explicit objects and amounts ("the £1,096 Hurlands pro forma", never "the invoice" or "he"), since the reader will have none of this conversation's context. Focus on key topics discussed, decisions made, and important context. Keep what was learned about the people involved along the way: how they felt, what they preferred, what their situation was at the time. Pinned facts hold only the durable truths; this note is where that surrounding context lives. CRITICAL: Frame EVERYTHING as completed/past - use past tense throughout. Never leave action items sounding open or pending. Be concise.\n\n${condensed}`,
      }],
    });
...
${numbered}`,
      ...
      const reply = responseText(response)?.trim().toLowerCase() || "";
      ...
      for (let c = 0; c < textCandidates.length; c++) {
        ...
        try {
          ...
          block.content = `[Compressed] ${responseText(summary) || block.content.substring(0, 200)}`;
          compressed++;
        } catch (e) {
  ...
  if (conversation.length > 3) {
    try {
      ...
      if (condensed.length > 50) {
        ...
        const summary = responseText(resp) || "";
        if (summary) {
...
async function _generateTitle(userId, conversation) {
  try {
    ...
    const title = (responseText(resp) || "").replace(/<think>[\s\S]*?<\/think>/g, "").trim();
