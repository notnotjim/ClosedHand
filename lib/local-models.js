...
// Roles whose files are already on disk. A boot after the first download
// still fires progress events while the files are read from the cache, and
// they used to be written as "downloading", so every bot restart showed the
// dashboard's "Getting to know your life" card with the recall model at
// 100%. A warm load is state "loading", which nothing displays.
const _warm = new Set();
async function _wasDownloaded(role) {
  try {
    const { getConf } = require("./config");
    const cur = (await getConf("LOCAL_MODELS_STATUS")) || {};
    return !!(cur[role] && (cur[role].downloaded || cur[role].state === "ready"));
...
async function ensureLocalEmbedder() {
  if (await _wasDownloaded("embedder")) _warm.add("embedder");
  await _writeStatus("embedder", { state: _warm.has("embedder") ? "loading" : "downloading", model: EMBED_ID(), pct: _warm.has("embedder") ? 100 : 0 }, true);
  try {
    await _call("preload", { which: "embedder" });
    await _writeStatus("embedder", { state: "ready", pct: 100, error: null, downloaded: true }, true);
  } catch (e) {
...
async function ensureLocalReranker() {
  if (await _wasDownloaded("reranker")) _warm.add("reranker");
  await _writeStatus("reranker", { state: _warm.has("reranker") ? "loading" : "downloading", model: RERANK_ID(), pct: _warm.has("reranker") ? 100 : 0 }, true);
  try {
    await _call("preload", { which: "reranker" });
    await _writeStatus("reranker", { state: "ready", pct: 100, error: null, downloaded: true }, true);
  } catch (e) {
