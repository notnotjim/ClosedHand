...
function queueUserMessage(userId, handler, options = {}) {
  ...
  // Housekeeping (a welcome check, a pinned message on reconnect) takes the
  // queue but is not a message: it must not make a running answer yield, or
  // a dropped connection reads as "the user said something new".
  const counts = !options.housekeeping;
  if (!module.exports.pendingIncoming) module.exports.pendingIncoming = {};
  if (counts) module.exports.pendingIncoming[userId] = (module.exports.pendingIncoming[userId] || 0) + 1;
  const wrapped = async () => {
    if (counts) module.exports.pendingIncoming[userId] = Math.max(0, (module.exports.pendingIncoming[userId] || 1) - 1);
    return handler();
  };
