...
const { unchanged } = require("./recall-settings");
const INTERVAL_MS = 15 * 60 * 1000;
const inflight = new Map();
const hash = text => crypto.createHash("sha256").update(text).digest("hex");
const dedicated = service => /^(google|microsoft)(_|$)/.test(service) || ["slack", "notion", "imap", "ics_calendar", "whatsapp", "whatsapp_linked", "telegram"].includes(service);
...
function createConnector({ db, indexItems, mcp, decryptTokens, encryptTokens, request = requestJson, now = Date.now, env = process.env }) {
  ...
  async function saveState(userId, source, row, state) {
    ...
    const query = db.from(table).update({ [field]: value }).eq("user_id", userId).eq("id", row.id);
    const { data: written, error: writeError } = await unchanged(query, field, live[field]).select("id");
  ...
  async function syncConnectedServices(userId) {
    ...
    const run = (async () => {
      ...
      for (const row of connections) {
        ...
        if (dedicated(row.service) && !Object.keys(discovery.descriptorFor(row)).length) continue;
