...
function createConnector({ db, indexItems, mcp, decryptTokens, encryptTokens, request = requestJson, now = Date.now, env = process.env }) {
  ...
  async function tokenFor(userId, conn) {
    ...
    if (tokens?.expiry && Number(tokens.expiry) <= now() + 60000 && tokens.refresh_token) {
      ...
      const clientId = tokens.client_id || env[`${refresh.envPrefix}_CLIENT_ID`], clientSecret = tokens.client_secret || env[`${refresh.envPrefix}_CLIENT_SECRET`];
  ...
  async function* resources(client, name, beforeRead) {
  ...
  async function syncSource(userId, row, source, previous) {
    ...
    try {
      ...
      invoke = async (op, args) => { await beforeRead(); return sourceInvoke(op, args); };
      ...
      if (resourceReader) {
        const seen = new Set();
        try {
          for await (const batch of resources(opened.client, row.name, beforeRead)) await saveBatch(userId, source, row, batch, old, seen, "resources");
          await ensureCurrent(userId, source, row);
