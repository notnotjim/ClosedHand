...
async function list(db, userId) {
  const sources = [];
  const { data: progress, error: progressError } = await db.from("index_progress").select("service,status,updated_at").eq("user_id", userId);
  if (progressError) throw new Error("Could not read source progress");
  const states = new Map((progress || []).map(s => [s.service, s]));
  for (const kind of ["connection", "mcp"]) {
    ...
    for (let offset = 0; ; offset += 500) {
      ...
      for (const row of data || []) {
        ...
        const latest = states.get(`recall:${where.source(row)}`);
        const failed = !existing && latest?.status === "error";
        sources.push({ kind, id: row.id, name: row.name || row.service, enabled, status: enabled ? existing ? "existing_reader" : failed ? "error" : state?.status || "pending" : "disabled",
          checked_at: failed ? latest.updated_at : state?.checked_at, reason: existing ? "This connection uses its existing recall sync. Check its source controls for progress." : failed ? "The latest sync did not finish. Check source access and try again." : state?.reason, collections: state?.collections || [], omitted: state?.plan?.skipped || [],
