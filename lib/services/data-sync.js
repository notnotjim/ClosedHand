...
const { isGoogleConnected, googleApiRequest, loadGoogleTokens, listGoogleAccounts } = require("./google");
...
async function upsertCacheItems(userId, source, type, items) {
  ...
  // Embed emails for semantic search (subject + sender + snippet)
  let embeddings = {};
  if (type === "email") {
    try {
      const { embedQuery } = require("../rag");
      // Batch embed: build text for each item, embed in batches of 20
      const textsToEmbed = items.map(item => {
        const from = item.from || item.sender || "";
        const subject = item.subject || "";
        const snippet = (item.body || item.snippet || "").substring(0, 150);
        return `From: ${from} Subject: ${subject} ${snippet}`.substring(0, 500);
      });
      for (let i = 0; i < textsToEmbed.length; i++) {
        try {
          const emb = await embedQuery(textsToEmbed[i]);
      ...
      if (Object.keys(embeddings).length > 0) {
        console.log(`[data-sync] Embedded ${Object.keys(embeddings).length}/${items.length} ${source} emails`);
      }
    } catch (e) {
      console.log(`[data-sync] Email embedding skipped: ${e.message}`);
    }
  }
  ...
  const rows = items.map((item, idx) => {
    ...
    if (embeddings[idx]) row.embedding = JSON.stringify(embeddings[idx]);
    return row;
  });
...
async function queryCacheEmails(userId, query, days = 7) {
  try {
    ...
    // Try semantic search for specific queries (not generic "show me recent")
    if (!isGeneric) {
      try {
        const { embedQuery } = require("../rag");
        const queryEmb = await embedQuery(query);
        if (queryEmb) {
          const { data: semanticMatches } = await supabase.rpc("match_cached_emails", {
          ...
          if (semanticMatches && semanticMatches.length > 0) {
            console.log(`[data-sync] Semantic email search: ${semanticMatches.length} matches for "${query.substring(0, 50)}"`);
            results = semanticMatches.map(m => ({ ...m.data, source: m.source, _similarity: m.similarity }));
            // Specific matches stay valid far longer than the 30-min "anything
            // new?" freshness window: historical emails don't go stale.
            return { results, cacheAgeMin, fresh: cacheAgeMin < 1440, source: "data_cache_semantic" };
          }
        }
      } catch (e) {
        console.log(`[data-sync] Semantic email search failed: ${e.message}`);
      }
    }

    let matchedSpecific = false;
    if (!isGeneric) {
      const stopWords = new Set(["the","and","or","from","for","about","with","that","this","has","was","are","my","me","in","on","at","to","of","a","an","is","it","by","up","do","no","if","so","email","emails","inbox","check","search","find","any","all","new","recent","unread"]);
      const keywords = query.toLowerCase().split(/\s+/).filter(w => w.length > 2 && !stopWords.has(w));

      if (keywords.length > 0) {
        const filtered = results.filter(item => {
          const attNames = (item.attachments || []).map(a => a.name || a.filename || "").join(" ");
          const text = ((item.subject || "") + " " + (item.from || "") + " " + (item.body || "") + " " + attNames).toLowerCase();
        ...
        if (filtered.length > 0) {
          ...
          // Specific query, zero matches in cache: report a miss so the
          // caller falls to live search, instead of returning a pile of
          // unrelated recent emails as a fake "hit".
          return null;
        }
      }
    }
    ...
    return {
      results,
      cacheAgeMin,
      // Specific matches don't go stale like "anything new?" queries do
      fresh: cacheAgeMin < (matchedSpecific ? 1440 : 30),
      source: "data_cache",
    };
  } catch (e) {
...
async function _searchCacheGeneric(userId, query, source, days, maxResults, excludeType) {
  try {
    ...
    if (query && query !== "*") {
      const stopWords = new Set(["the","and","or","from","for","about","with","that","this","has","was","are","my","me","in","on","at","to","of","a","an","is","it"]);
      const keywords = query.toLowerCase().split(/\s+/).filter(w => w.length > 2 && !stopWords.has(w));
      if (keywords.length > 0) {
        const filtered = results.filter(item => {
