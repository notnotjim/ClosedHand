...
async function discoverHttp(row, request, fallback) {
  ...
  for (const origin of origins.slice(0, 3)) {
    for (const path of ["/.well-known/openapi.json", "/openapi.json", "/swagger.json"]) {
      ...
      } catch (e) { if (![404, 403, 401].includes(e.status)) failure = e; }
    }
  }
  if (failure || stateFor(row)?.plan?.recipes?.length) throw new Error("The API description could not be read; existing records are kept for the next attempt.");
