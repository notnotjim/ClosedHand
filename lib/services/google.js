...
// A 403 for missing scopes is as permanent as a dead refresh token: the grant
// itself is short, so every retry asks for something this token can never be
// given. Left unflagged it fails on a five minute loop for as long as the
// connection exists, which is how one account spent four days doing nothing but
// filling the log with the same 403. Only the core reads should call this; a
// 403 on drafts means the connection merely lacks the compose scope and the
// rest of it still works.
function isMissingScopeError(e) {
  const msg = e?.message || "";
  return /\b403\b/.test(msg) && /insufficient (authentication scopes|permission)/i.test(msg);
}

async function markGoogleScopeFailure(e, userStore, serviceKey = "google") {
  if (!isMissingScopeError(e)) return false;
  const store = userStore || ctx.activeUserStore;
  try {
    await store?.markConnectionReconnectRequired?.(serviceKey, "missing scopes");
  } catch (_) {}
  return true;
}
...
module.exports = {
  ...
  isMissingScopeError, markGoogleScopeFailure,
