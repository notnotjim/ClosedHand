// Built-in Kokoro speech. One bounded helper serves chat apps and web playback.
...
function createSpeechService(options = {}) {
  ...
  function stopWorker() {
    ...
    // A process boundary releases native ONNX allocations on Linux too.
    if (old) old.kill();
  }
  ...
  function dispatch() {
    if (active && ready) worker.send({ id: active.id, text: active.text, format: active.format });
  }
  function startWorker() {
    const instance = spawn(path.join(__dirname, "../speech/worker.js"), {
      serialization: "advanced", stdio: ["ignore", "ignore", "inherit", "ipc"],
      // Speech needs no provider credentials or user configuration.
