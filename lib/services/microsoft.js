...
function refreshMicrosoftToken(tokens, _userStore, serviceKey = "microsoft") {
  return new Promise((resolve, reject) => {
    if (!(tokens.client_id || MICROSOFT_CLIENT_ID) || !(tokens.client_secret || MICROSOFT_CLIENT_SECRET)) {
      return reject(new Error("Microsoft OAuth not configured (missing client ID/secret)"));
    }

    const postData = new URLSearchParams({
      client_id: tokens.client_id || MICROSOFT_CLIENT_ID,
      client_secret: tokens.client_secret || MICROSOFT_CLIENT_SECRET,
