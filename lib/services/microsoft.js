...
const { refreshRequest } = require("../microsoft-app");
...
function refreshMicrosoftToken(tokens, _userStore, serviceKey = "microsoft") {
  return new Promise((resolve, reject) => {
    const request = refreshRequest(tokens, "openid profile email offline_access Mail.ReadWrite Mail.Send Calendars.ReadWrite Files.Read.All");
    if (!request) {
      return reject(new Error("Microsoft OAuth not configured (missing client ID/secret)"));
    }

    const postData = new URLSearchParams(request.body).toString();

    const req = https.request(
      {
        hostname: "login.microsoftonline.com",
        path: `/${request.authority}/oauth2/v2.0/token`,
        method: "POST",
        headers: {
