...
async function searchCacheFn(userId, params) {
  ...
  if (!unread_only) {
    try {
      ...
      if (cached && cached.fresh && cached.results.length > 0) {
        ...
        // primary mailbox.
        let limited = cached.results.slice(0, Math.min(100, Math.max(1, max_results))).map(e => require("../otp-guard").redactEmail({
          id: e.id,
          ...
          subject: e.subject,
          date: e.date,
          body: require("./lexical").matchWindow(e.body || "", require("./lexical").lexicalTokens(query), 800),
          body_truncated: (e.body || "").length > 800,
          read_tool: "read_cached_record",
          mailbox: e.mailbox || "",
          attachments: e.attachments || [],
        }, userId));
  ...
  const limited = deduped.map(e => require("../otp-guard").redactEmail({
    id: e.id,
    source: e.source,
    account: e.account,
    thread_id: e.threadId,
