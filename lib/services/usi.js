...
// Words that carry no retrieval signal. Kept deliberately short: over-filtering
// costs recall, and ts_rank_cd already discounts terms that appear everywhere.
const LEX_STOP = new Set(["the","and","or","from","for","about","with","that","this","has","was","are","did","does","what","when","where","which","who","whom","why","how","any","all","some","said","say","says","tell","find","show","get","give","email","emails","mail","message","messages","please","can","you","your","my","me","our","their","his","her","its","have","had","been","were","will","would","should","could","there","here","them","they","she","not","but","out","see","look","need","want","know","about"]);
...
function lexicalTokens(query) {
  return String(query || "")
    ...
    .filter(w => w.length > 2 && !LEX_STOP.has(w.toLowerCase()))
    .slice(0, 12);
}

// A window of text centred on the first matching token, so the thing that
// caused the match is visible in the snippet. brain.js truncates un-enriched
// content at 400 characters, so a full body handed over raw would routinely cut
// off the identifier that made the row match at all.
