  ...
  while (true) {
    ...
    await Promise.allSettled(basics.map(item => limit(async () => {
      ...
      try {
        ...
        await supabase.from("data_vectors").update({
          content: summary,
...
async function search(userId, query, opts = {}) {
  // Query embed is interactive: fast-fail so a degraded embedder downgrades
  // the search to keyword fallback instead of stalling the user
  const embedding = await embedText(query, { quick: true });
  if (!embedding) return { results: [], error: "Semantic search temporarily unavailable. Fall back to search_cache keyword search now." };

  const { data: results, error } = await supabase.rpc("search_data_vectors", {
  ...
  if (allIndexedIds.size > 0) {
    ...
    if (toDelete.length > 0) {
      for (const v of toDelete) {
        await supabase.from("data_vectors").delete().eq("id", v.id);
      }
...
async function upsertProgress(userId, service, updates) {
  ...
  try { await supabase.from("index_progress").upsert(row, { onConflict: "user_id,service" }); } catch (_) {}
}
...
// A pass over a large mailbox on a local CPU embedder takes minutes, and the
// sync loop comes round every few. Without this guard each cycle piled another
// pass on top of the unfinished one; they fought for the same CPU and none of
// them ever reached its first write, so an install could sync for an hour and
// index nothing.
const _passRunning = new Set();

async function runPostSyncIndex(userId) {
  if (_passRunning.has(userId)) {
    console.log(`[USI] pass already running for ${userId}, skipping this cycle`);
    return;
  }
  _passRunning.add(userId);
