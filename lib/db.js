// lib/db.js — Lazy Supabase client so the process boots with no DB configured.
//
...
// throwing — so the app can enter setup mode rather than crashing. When the DB is
// configured, every call forwards to the real client unchanged.
//
// Webapp is a separate service and cannot import lib/, so it has its own copy at
// webapp/db.js — keep the two in sync (same constraint as crypto-tokens.js).
