...
async function fileBugReport({ userId, text, fileData, platform, chatId }) {
  try {
    ...
    if (isSelfHost()) {
      ...
      const carries = `your description, up to ${SNAPSHOT_TURNS} recent conversation entries${savedShots ? " and the saved screenshots" : ""}, plus the app version and a random ID for this ClosedHand`;
