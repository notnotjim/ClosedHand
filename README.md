...
The installer opens the setup page in your browser when the stack is up (or tells you the address, **http://localhost:3000**, if it cannot). It walks you through the rest and lights each step up as it detects you have done it: paste one model provider key, set an admin password, connect Google through a Connect to Google page that opens the exact console pages you need, then connect Telegram with a token from BotFather. The Google part is yours to do because ClosedHand runs on your machine rather than on a company's, and the page takes you through it step by step. By the time you say hello in Telegram, ClosedHand has already read your inbox and calendar, and the first conversation is usually about twenty minutes in.
